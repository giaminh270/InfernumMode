...
namespace InfernumMode
{
    public class Primitive3DStrip
    {
        ...
        internal Texture2D BandTexture;
        ...
        public void UseBandTexture(Texture2D bandTexture) => BandTexture = bandTexture;
        ...
        internal VertexPositionColorTexture[] CalculateVertices(Vector2 left, Vector2 right, float textureScrollSpeed, float verticalWobble, float wobblePhaseShift)
        {
            ...
            for (int i = 0; i < vertexCount; i++)
            {
                ...
                float x = Lerp(left.X, right.X, (float)Cos(angleCompletionRatio) * 0.5f + 0.5f);
                float z = (float)Sin(angleCompletionRatio + wobblePhaseShift);
                float y = Lerp(left.Y, right.Y, (float)Pow(Sin(angleCompletionRatio * 0.5f), 2f)) + z * verticalWobble;
                float height = HeightFunction(z * 0.5f + 0.5f);
                Color color = ColorFunction(z * 0.5f + 0.5f);

                Vector3 center = new Vector3(x, y, 1f);
                VertexPositionColorTexture top = new VertexPositionColorTexture(center - Vector3.UnitY * height, color, new Vector2((completionRatio + Main.GlobalTime * textureScrollSpeed) % 1f, 0f));
                VertexPositionColorTexture bottom = new VertexPositionColorTexture(center + Vector3.UnitY * height, color, new Vector2((completionRatio + Main.GlobalTime * textureScrollSpeed) % 1f, 1f));
        ...
        public void Draw(Vector2 left, Vector2 right, float textureScrollSpeed, float verticalWobble, float wobblePhaseShift)
        {
			...
			if (BandTexture != null)
                BaseEffect.Texture = BandTexture;
