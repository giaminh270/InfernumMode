...
namespace InfernumMode.UI
{
    public static class GuardiansPlaqueUIManager
    {
        ...
        public static Vector2 PlaqueWorldPosition => new Vector2(PoDWorld.ProvidenceArena.X * 16f + 4360f, PoDWorld.ProvidenceArena.Y * 16f + 1921f);

        // This not being immediate causes issues with getting it for initial sizings.
        public static Texture2D BackgroundTexture => ModContent.GetTexture("InfernumMode/ExtraTextures/UI/ProfanedPlaqueBackground");
        ...
        public static DynamicSpriteFont TextFont
        {
            get
            {
                ...
                if (Environment.OSVersion.Platform == PlatformID.Win32NT)
                    return ModContent.GetInstance<InfernumMode>().GetFont("Fonts/ProfanedText");

                return Main.fontMouseText;
            }
        }
        ...
        public static void Draw(SpriteBatch spriteBatch)
        {
            ...
            spriteBatch.End();
            spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.AnisotropicClamp, DepthStencilState.DepthRead, Main.instance.Rasterizer, null, Main.UIScaleMatrix);
            ...
            foreach (string line in Utils.WordwrapString(TextToDraw, TextFont, maxTextLength, 100, out _))
            {
                ...
                {
                    List<string> splitLines = line.Split(new[] { SpecialText }, StringSplitOptions.None).ToList();
        ...
        public static void DrawTextLine(string line, Vector2 textLeftDrawPosition, SpriteBatch spriteBatch)
        {
            ...
            Rectangle textRectangle = new Rectangle((int)textLeftDrawPosition.X, (int)textLeftDrawPosition.Y + 5, (int)textArea.X, (int)(0.667f * textArea.Y));
            if (line == SpecialText)
            {
                ...
                if (Utils.CenteredRectangle(Main.MouseScreen, Vector2.One).Intersects(textRectangle))
                {
                    ...
                    Main.HoverItem.SetDefaults(ModContent.ItemType<ProfanedShard>());
