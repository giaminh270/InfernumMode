...
namespace InfernumMode.BehaviorOverrides.BossAIs.Prime
{
    public abstract class PrimeHandBehaviorOverride : NPCBehaviorOverride
    {
        ...
        public override bool PreAI(NPC npc)
        {
            ...
            if (npc.type == NPCID.PrimeCannon || npc.type == NPCID.PrimeLaser)
                npc.damage = 0;
        ...
        public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            if (telegraphIntensity > 0f)
            {
                ...
                Color outlineColor = Color.Lerp(TelegraphColor, Color.White, telegraphIntensity) * Utils.InverseLerp(1f, 0.7f, telegraphIntensity, true);
            ...
            if (telegraphIntensity > 0f)
            {
                float opacity = Utils.InverseLerp(0f, 0.25f, telegraphIntensity, true) * Utils.InverseLerp(1f, 0.9f, telegraphIntensity, true);
                Color backglowColor = Color.Lerp(Color.OrangeRed, Color.Red, 0.75f);
                for (int i = 0; i < 8; i++)
                {
            ...
            if (npc.ai[3] == 0f && npc.type == NPCID.PrimeLaser)
                Main.spriteBatch.Draw(Main.BoneLaserTexture, drawPosition, npc.frame, new Color(200, 200, 200, 0), npc.rotation, npc.frame.Size() * 0.5f, npc.scale, SpriteEffects.None, 0f);
            return false;
        }
