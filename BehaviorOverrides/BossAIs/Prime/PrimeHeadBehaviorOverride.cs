...
namespace InfernumMode.BehaviorOverrides.BossAIs.Prime
{
    public class PrimeHeadBehaviorOverride : NPCBehaviorOverride
    {
        ...
        public static Dictionary<int, Vector2> ArmPositionOrdering => new Dictionary<int, Vector2>()
        {
            [NPCID.PrimeLaser] = new Vector2(-350f, 200f),
            [NPCID.PrimeSaw] = new Vector2(-200f, 150f),
            [NPCID.PrimeVice] = new Vector2(200f, 150f),
            [NPCID.PrimeCannon] = new Vector2(350f, 200f),
        };
        ...
        public static void DoBehavior_SpawnEffects(NPC npc, Player target, float attackTimer, ref float frameType)
        {
            ...
            if (npc.WithinRange(Main.LocalPlayer.Center, 3700f) && CanPerformCameraEffects)
            {
                ...
                Main.LocalPlayer.Infernum().ScreenFocusInterpolant *= Utils.InverseLerp(animationTime, animationTime - 8f, attackTimer, true);
            }
            ...
            if (canHover)
            {
                ...
                npc.velocity = npc.SafeDirectionTo(hoverDestination) * Math.Min(npc.Distance(hoverDestination), 32f);
        ...
        public static void DoBehavior_MetalBurst(NPC npc, Player target, float attackTimer, ref float frameType)
        {
            ...
            {
                spikesPerBurst += 10;
                hoverSpeed = Math.Max(hoverSpeed, 30f) * 1.2f;
            }
        ...
        public static void DoBehavior_RocketRelease(NPC npc, Player target, float lifeRatio, float attackTimer, ref float frameType)
        {
            ...
            if (wrappedTime > cycleTime - rocketCountPerCycle * 2f && attackTimer > rocketShootDelay)
            {
                ...
                {
                	Main.PlaySound(InfernumSoundRegistry.ApolloMissileLaunch, npc.Center);
        ...
        public static void DoBehavior_HoverCharge(NPC npc, Player target, float lifeRatio, float attackTimer, ref float frameType)
        {
            ...
            if (wrappedTime < hoverTime - 15f)
            {
                ...
                npc.velocity = (npc.velocity * 7f + npc.SafeDirectionTo(hoverDestination) * Math.Min(npc.Distance(hoverDestination), hoverSpeed)) / 8f;
            ...
            else
            {
                if (wrappedTime == hoverTime + 1f)
                {
                    ...
                    if (!AnyArms)
                    {
                        ...
                        Main.PlaySound(InfernumSoundRegistry.PrimeChargeSound, target.Center);

                        // Release some smoke backwards.
                        for (int i = 0; i < 32; i++)
                        {
                            Color smokeColor = Color.Lerp(Color.Orange, Color.Red, Main.rand.NextFloat(0.65f));
                            SmallSmokeParticle smoke = new SmallSmokeParticle(npc.Center + Main.rand.NextVector2Circular(90f, 90f), -npc.velocity.RotatedByRandom(0.52f) * Main.rand.NextFloat(0.3f, 2f), smokeColor, Color.DarkGray, 1f, 255f, Main.rand.NextFloat(0.01f));
                            GeneralParticleHandler.SpawnParticle(smoke);
                        }
                        for (int i = 0; i < 20; i++)
                        {
                            Color smokeColor = Color.Lerp(Color.Orange, Color.Red, Main.rand.NextFloat(0.55f));
                            MediumMistParticle smoke = new MediumMistParticle(npc.Center + Main.rand.NextVector2Circular(90f, 90f), -npc.velocity.RotatedByRandom(0.52f) * Main.rand.NextFloat(0.2f, 1.2f), smokeColor, Color.White, 1f, 255f, Main.rand.NextFloat(0.01f));
                            GeneralParticleHandler.SpawnParticle(smoke);
                        }
                    }
                }
        ...
        public static void DoBehavior_LightningSupercharge(NPC npc, Player target, ref float attackTimer, ref float frameType)
        {
            ...
            else if (struckByLightningFlag == 1f)
            {
                ...
                npc.velocity = (npc.velocity * 6f + npc.SafeDirectionTo(hoverDestination) * Math.Min(npc.Distance(hoverDestination), movementSpeed)) / 7f;
                npc.rotation = npc.velocity.X * 0.04f;
        ...
        public static void SelectNextAttack(NPC npc)
        {
            ...
            if (oldAttack == PrimeAttackType.SynchronizedMeleeArmCharges || oldAttack == PrimeAttackType.SlowSparkShrapnelMeleeCharges)
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                ...
                if (n.type != NPCID.PrimeCannon && n.type != NPCID.PrimeLaser && n.type != NPCID.PrimeSaw && n.type != NPCID.PrimeVice)
                    continue;
        ...
        public static void GetCannonAttributesByAttack(PrimeAttackType attackState, out int telegraphTime, out int totalShootTime, out int shootCycleTime)
        {
            ...
            if (attackState == PrimeAttackType.SynchronizedMeleeArmCharges || attackState == PrimeAttackType.SlowSparkShrapnelMeleeCharges)
            {
        ...
        public static bool CannonCanBeUsed(NPC cannon, out float telegraphInterpolant)
        {
			...
			bool meleeCannon = cannon.type == NPCID.PrimeVice || cannon.type == NPCID.PrimeSaw;
			bool rangedCannon = cannon.type == NPCID.PrimeLaser || cannon.type == NPCID.PrimeCannon;
        ...
        public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            Texture2D eyeGlowTexture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/Prime/PrimeEyes");
