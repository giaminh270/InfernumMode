...
namespace InfernumMode.BehaviorOverrides.BossAIs.AquaticScourge
{
    public class AquaticScourgeHeadBehaviorOverride : NPCBehaviorOverride
    {
        ...
        private void UpdateAcidHissSound(NPC npc, int hitDirection, double damage)
        {
            ...
            bool aquaticScourge = npc.type == ModNPCType<AquaticScourgeHead>() || npc.type == ModNPCType<AquaticScourgeBody>() || npc.type == ModNPCType<AquaticScourgeTail>();
        ...
        public static List<VerletSimulatedSegmentInfernum> WormSegments
        {
            get;
            set;
        } = new List<VerletSimulatedSegmentInfernum>();
        ...
        public override bool PreAI(NPC npc)
        {
            ...
            if (calamityModMusic != null)
                npc.modNPC.music = calamityModMusic.GetSoundSlot(Terraria.ModLoader.SoundType.Music, "Sounds/Music/AquaticScourge");
            else
            ...
            if (attackTimer != (int)AquaticScourgeAttackType.SpawnAnimation && target.Infernum().SulphWaterPoisoningLevel >= 0.5f && (acidMeterEverReachedHalf == 0f || enraged))
            {
            ...
            if (acidVerticalLine != 0f && attackType != (int)AquaticScourgeAttackType.DeathAnimation)
            {
                if (Main.rand.NextBool(3))
                {
                    ...
                    Vector2 skullSpawnPosition = new Vector2(target.Center.X + Main.rand.NextFloatDirection() * 800f, skullSpawnPositionY);
                ...
                for (int i = 0; i < 36; i++)
                {
                    ...
                    Vector2 acidSpawnPosition = new Vector2(target.Center.X + Main.rand.NextFloatDirection() * 1100f, acidSpawnPositionY);
                    ...
                    if (t.active() || t.liquid <= 0)
                        continue;

                    CloudParticle acidFoam = new CloudParticle(acidSpawnPosition, acidVelocity, acidColor, Color.White, 20, 0.3f)
                    {
        ...
        public static void DoBehavior_SpawnAnimation(NPC npc, Player target, ref float attackTimer)
        {
            ...
            if (attackTimer < waterBubbleTime)
            {
                ...
                float cameraInterpolant = Utils.InverseLerp(acidWarningDelay, acidWarningDelay + 10f, attackTimer, true) * Utils.InverseLerp(waterBubbleTime - 1f, waterBubbleTime - 10f, attackTimer, true);
                if (bubbleProj != null)
                {
                    target.Infernum().ScreenFocusInterpolant = cameraInterpolant;
                    target.Infernum().ScreenFocusPosition = bubbleProj.Center;
                }
            }
            ...
            {
                float bubbleSpawnRate = Utilities.Remap(attackTimer, 0f, acidWarningDelay + 45f, 0.02f, 0.3f);
                for (int i = 0; i < 3; i++)
                {
                    ...
                    if (bubbleProj != null && Vector2.Distance(bubbleSpawnPosition, bubbleProj.Center) < bubbleProj.scale * 300f)
                        continue;
                    ...
                    Gore bubble = Gore.NewGorePerfect(bubbleSpawnPosition, -Vector2.UnitY * Main.rand.NextFloat(0.6f, 3f), goreID);
                    bubble.type = goreID;
                    bubble.timeLeft = 50;
                    bubble.scale = 0.4f;
                    ...
                    for (int j = 0; j < 3; j++)
                    {
                        ...
                        acidVelocity.X += CalamityWorld.abyssSide.ToDirectionInt() * -4f;
            ...
            if (attackTimer >= waterBubbleTime)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    if (bubbleProj != null && Vector2.Distance(acidSpawnPosition, bubbleProj.Center) < bubbleProj.scale * 300f)
                        continue;

                    MediumMistParticle acidFoam = new MediumMistParticle(acidSpawnPosition, -Vector2.UnitY.RotatedByRandom(0.67f) * Main.rand.NextFloat(4f, 20f), acidColor, Color.White, 0.5f, 255f, 0.02f);
                    GeneralParticleHandler.SpawnParticle(acidFoam);
                }
            }
            ...
            {
                Main.PlaySound(InfernumSoundRegistry.AquaticScourgeAppearSound);
                Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(Terraria.ModLoader.SoundType.Custom, "Sounds/Custom/MaulerRoar"));
            ...
            if (attackTimer == waterBubbleTime + acidFizzleTime + 34f)
            {
                ...
                for (int i = 0; i < 100; i++)
                {
                    ...
                    MediumMistParticle acidMistFoam = new MediumMistParticle(acidSpawnPosition, acidVelocity, acidColor, Color.White, 2f, 255f, 0.009f);
        ...
        public static void DoBehavior_BubbleSpin(NPC npc, Player target, bool phase2, bool enraged, ref float attackTimer)
        {
            ...
            {
                float flySpeed = Utilities.Remap(attackTimer, 16f, redirectTime - 8f, 11f, spinSpeed);
            ...
            if (attackTimer >= redirectTime + spinTime && !charging)
            {
                ...
                if (attackTimer == redirectTime + spinTime)
                {
                    Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(Terraria.ModLoader.SoundType.Custom, "Sounds/Custom/DesertScourgeRoar"), target.Center);
                    PopAllBubbles();
                }
                ...
                {
                    Main.PlaySound(InfernumSoundRegistry.AquaticScourgeChargeSound, target.Center);
        ...
        public static void DoBehavior_WallHitCharges(NPC npc, Player target, bool phase2, bool enraged, ref float attackTimer)
        {
            ...
            {
                float slowdownInterpolant = (float)Math.Pow(attackTimer / chargeDelay, 2f);
            ...
            if (performingCharge == 1f)
            {
                ...
                if (attackTimer >= minChargeTime && dontInteractWithBlocksYet == 0f && insideBlocks && npc.WithinRange(target.Center, 1200f))
                {
                    ...
                    {
                        Main.PlaySound(InfernumSoundRegistry.SkeletronHeadBonkSound, target.Center);
                    }
                    else
        ...
        public static void DoBehavior_EnterSecondPhase(NPC npc, Player target, ref float attackTimer)
        {
            ...
            if (attackTimer == roarDelay)
            {
                ...
                Main.LocalPlayer.Infernum().CurrentScreenShakePower = 8f;
            ...
            target.Infernum().SulphWaterPoisoningLevel = 0f;
        ...
        public static void DoBehavior_PerpendicularSpikeBarrage(NPC npc, Player target, bool enraged, ref float attackTimer)
        {
            ...
            if (attackTimer == slowdownTime)
            {
                ...
                Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(Terraria.ModLoader.SoundType.Custom, "Sounds/Custom/MaulerRoar"), target.Center);
                Main.PlaySound(InfernumSoundRegistry.AquaticScourgeGoreSound, target.Center);
            ...
            if (shudderOffset > 0f)
            {
                segmentShudderIndex = Utils.InverseLerp(slowdownTime, slowdownTime + attackTime - 10f, attackTimer, true) * 35f;
                shudderOffset -= 0.15f;
        ...
        public static void DoBehavior_GasBreath(NPC npc, Player target, bool phase2, bool phase3, bool enraged, ref float attackTimer)
        {
            ...
            float movementSpeed = MathHelper.Lerp(14.5f, 38f, Utils.InverseLerp(640f, 3000f, npc.Distance(target.Center), true));
            ...
            if (goodBubbles.Any())
            {
                ...
                float closenessInterpolant = Utils.InverseLerp(400f, 180f, closestBubble.Distance(target.Center), true);
            ...
            if (!npc.WithinRange(target.Center, 240f))
            {
                ...
                {
                    Main.PlaySound(InfernumSoundRegistry.DesertScourgeShortRoar, npc.Center);
        ...
        public static void DoBehavior_EnterFinalPhase(NPC npc, Player target, ref float attackTimer, ref float acidVerticalLine)
        {
            ...
            {
                acidVerticalLine = (float)Math.Min(target.Bottom.Y + 2000f, SulphurousSea.YStart * 16f + 900f);
            ...
            ActiveSound rumbleSound = Main.GetActiveSound(SlotId.FromFloat(rumbleSoundSlot));
            bool soundIsValid = rumbleSound != null && rumbleSound.IsPlaying;
            bool needsToPlayRumble = !soundIsValid || rumbleSoundSlot == 0f;
            float rumbleVolume = Utils.InverseLerp(acidVerticalLine - 2900f, acidVerticalLine - 1650f, target.Center.Y, true) * Utils.InverseLerp(lineTop + 300f, lineTop + 800f, acidVerticalLine, true) * 1.8f + 0.001f;
            if (needsToPlayRumble && acidVerticalLine >= lineTop + 300f)
                rumbleSoundSlot = Main.PlayTrackedSound(InfernumSoundRegistry.LeviathanRumbleSound, target.Center).ToFloat();
            if (rumbleSound != null)
                rumbleSound.Volume = rumbleVolume;

            // Make the acid rise upward.
            if (acidVerticalLine >= lineTop)
                acidVerticalLine -= Utilities.Remap(acidVerticalLine, (float)lineTop + 500f, (float)lineTop, 10.5f, 4f);
            else
            {
        ...
        public static void DoBehavior_AcidRain(NPC npc, Player target, ref float attackTimer)
        {
            ...
            switch ((int)attackSubstate)
            {
                ...
                case 0:
                    ...
                    {
                        Vector2 idealVelocity = new Vector2(npc.SafeDirectionTo(target.Center).X * 27f, -verticalSpeedAdditive - 24f);
        ...
        public static void DoBehavior_DeathAnimation(NPC npc, Player target, ref float jawRotation, ref float attackTimer, ref float skullWasTaken)
        {
            ...
            {
                float approachSpeed = Utilities.Remap(attackTimer, 0f, goreSpitTime - 30f, 7f, 0.8f);
            ...
            if (goreSpitCountdown <= 0f && timeAfterSpitting == 0f)
            {
                ...
                for (int i = 0; i < 9; i++)
                {
                    float bloodScale = Main.rand.NextFloat(0.75f, 1.1f) * Utilities.Remap(attackTimer, 0f, goreSpitTime - 45f, 1f, 1.24f);
                    Color darkRed = Color.Lerp(Color.DarkRed, Color.Black, Main.rand.NextFloat(0.25f, 0.55f));
            ...
            float idealJawRotation = Utilities.Remap(goreSpitCountdown, 75f, 27f, 0f, MathHelper.Pi / 5f);
            ...
            if (timeAfterSpitting != 0f)
            {
                ...
                if (hasCreatedSplashSound == 0f)
                {
                    ...
                    if (totalMovingSegmentsInWater >= 7f)
                    {
                        ...
                        Main.PlaySound(InfernumSoundRegistry.AquaticScourgeAppearSound, target.Center);
                ...
                {
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                        Item.NewItem(target.Hitbox, ModContent.ItemType<AquaticScourgeSkull>());
            ...
            {
                deadTimer++;
                npc.Opacity = Utils.InverseLerp(disappearTime, 0f, deadTimer, true);
                if (npc.Opacity <= 0f)
                    npc.active = false;
            }
        }
        ...
        {
            WormSegments.Clear();
            WormSegments.Add(new VerletSimulatedSegmentInfernum(npc.Center, Vector2.UnitY * 0.25f, true));
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                ...
                foreach (Vector2 spikePosition in AquaticScourgeBodyBehaviorOverride.GetSpikePositions(n))
                {
                    ...
                    for (int j = 0; j < 3; j++)
                    {
                        ...
                        bloodVelocity.Y -= 12f;
                        BloodParticle blood = new BloodParticle(spikePosition, bloodVelocity, bloodLifetime, bloodScale, bloodColor);
                    ...
                    for (int j = 0; j < 2; j++)
                    {
                        ...
                        BloodParticle2 blood = new BloodParticle2(spikePosition, bloodVelocity, 20, bloodScale, bloodColor);
                        GeneralParticleHandler.SpawnParticle(blood);
                    }
                }

                CloudParticle bloodCloud = new CloudParticle(n.Center, Main.rand.NextVector2Circular(12f, 12f), Color.Red, Color.DarkRed, 270, Main.rand.NextFloat(1.9f, 2.12f));
        ...
        public static void ApplySulphuricPoisoningBoostToPlayersInArea(Vector2 center, float radius, float boostFactor)
        {
            for (int i = 0; i < Main.maxPlayers; i++)
            {
                ...
                {
                    float increment = boostFactor / PoDPlayer.SulphSeaWaterSafetyTime;
                    if (p.Calamity().sulphurskin)
                        increment *= 0.5f;
                    if (p.Calamity().sulfurSet)
                        increment *= 0.5f;
                    p.Infernum().SulphWaterPoisoningLevel += increment;
                }
            }
        }
        ...
        public static void UpdateAcidHissSound(NPC npc)
        {
            ...
            float volumeInterpolant = Utils.InverseLerp(500f, 60f, MathHelper.Distance(target.Center.Y, acidVerticalLine), true);
            if (target.Center.Y >= acidVerticalLine)
        ...
        public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            float jawRotationSine = (float)Math.Sin(jawRotation);
            ...
            Texture2D headTexture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/AquaticScourge/AquaticScourgeHead");
            Texture2D jawTexture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/AquaticScourge/AquaticScourgeJaw");
            Texture2D headTextureSkeleton = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/AquaticScourge/AquaticScourgeHeadSkeleton");
            Vector2 origin = headTexture.Size() * 0.5f;
            Main.spriteBatch.Draw(headTexture, drawPosition, null, npc.GetAlpha(lightColor) * (1f - skeletonInterpolant), npc.rotation, origin, npc.scale, 0, 0);
            Main.spriteBatch.Draw(headTextureSkeleton, drawPosition, null, npc.GetAlpha(lightColor) * skeletonInterpolant, npc.rotation, origin, npc.scale, 0, 0);
            ...
            Main.spriteBatch.Draw(jawTexture, leftJawDrawPosition, null, npc.GetAlpha(lightColor), leftJawRotation, leftJawOrigin, npc.scale, SpriteEffects.None, 0);
            Main.spriteBatch.Draw(jawTexture, rightJawDrawPosition, null, npc.GetAlpha(lightColor), rightJawRotation, rightJawOrigin, npc.scale, SpriteEffects.FlipHorizontally, 0);
