...
namespace InfernumMode.BehaviorOverrides.BossAIs.StormWeaver
{
    public class StormWeaverHeadBehaviorOverride : NPCBehaviorOverride
    {
        ...
        public static void DoBehavior_HuntSkyCreatures(NPC npc, Player player, ref float attackTimer, ref float lightningSkyBrightness, ref float fogInterpolant)
        {
            ...
            if (attackTimer == 1f)
            {
                Main.PlaySound(InfernumSoundRegistry.CalThunderStrikeSound, player.Center);
                Main.PlaySound(InfernumSoundRegistry.StormWeaverElectricDischargeSound, player.Center);
                npc.netUpdate = true;
            ...
            {
                // Play a slice sound.
                Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(Terraria.ModLoader.SoundType.Custom, "Sounds/Custom/SwiftSlice"), target.Center);
        ...
        public static void CreateImpactParticlesForTarget(NPC npc, NPC target)
        {
            if (target.Organic())
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    BloodParticle2 blood = new BloodParticle2(target.Center, bloodVelocity, 20, bloodScale, bloodColor);
                    GeneralParticleHandler.SpawnParticle(blood);
                }
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                sparkVelocity.Y -= 4f;
                SparkParticle spark = new SparkParticle(target.Center, sparkVelocity, sparkLifetime, sparkScale, sparkColor);
        ...
        public static void DoBehavior_FakeoutCharge(NPC npc, Player target, ref float lightningSkyBrightness, ref float attackTimer)
        {
            ...
            switch (substate)
            {
                case 0:
                    ...
                    float flySpeed = MathHelper.Lerp(27 * 0.5f, 27, Utils.InverseLerp(50f, 270f, npc.Distance(hoverDestination), true));
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (npc.WithinRange(new Vector2(centerX, centerY), initialChargeStopDistance) || attackTimer > 90f)
                    {
                        npc.velocity *= 0.95f;
        ...
        public static void DoBehavior_FogSneakAttackCharges(NPC npc, Player target, ref float lightningSkyBrightness, ref float attackTimer, ref float fogInterpolant, ref float electricityFormInterpolant)
        {
            ...
            if (attackTimer <= telegraphTime)
            {
                ...
                {
                    Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/PlasmaBolt"), teleportPosition);

                    CreateSparks(teleportPosition);
                    for (int i = 0; i < 16; i++)
                    {
                        Color fireColor = Main.rand.NextBool() ? Color.Yellow : Color.Cyan;
                        CloudParticle fireCloud = new CloudParticle(teleportPosition, (MathHelper.TwoPi * i / 16f).ToRotationVector2() * 6f, fireColor, Color.DarkGray, 20, Main.rand.NextFloat(2.5f, 3.2f));
            ...
            else if (attackTimer == telegraphTime + 1f)
            {
                ...
                Main.PlaySound(InfernumSoundRegistry.CalThunderStrikeSound, target.Center);
                Main.PlaySound(InfernumSoundRegistry.StormWeaverElectricDischargeSound, target.Center);
        ...
        public static void DoBehavior_AimedLightningBolts(NPC npc, Player target, ref float attackTimer, ref float lightningSkyBrightness, ref float electricityFormInterpolant)
        {
            ...
            if (attackTimer <= redirectTime)
            {
                ...
                float movementInterpolant = Utilities.Remap(attackTimer, 0f, redirectTime - 20f, 0.2f, 0.04f);
            ...
            if (attackTimer == redirectTime + arcRedirectTime + 1f)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    float aimAtTargetInterpolant = Utilities.Remap(npc.Distance(target.Center), 720f, 1400f, 0.1f, 0.85f);
                    for (int i = 0; i < Main.maxNPCs; i++)
                    {
                        NPC n = Main.npc[i];
        ...
        public static void DoBehavior_BerdlyWindGusts(NPC npc, Player target, ref float attackTimer)
        {
            ...
            Vector2 windSpawnPosition = target.Center + new Vector2(Main.windSpeedTemp.DirectionalSign() * -1250f, Main.rand.NextFloatDirection() * 900f);
            Vector2 windVelocity = new Vector2(1f, 0.25f) * Main.windSpeedTemp.DirectionalSign() * Main.rand.NextFloat(0.1f, 1.8f) * 70f;
            Particle wind = new SnowyIceParticle(windSpawnPosition, windVelocity, Color.White * 0.6f, Main.rand.NextFloat(0.7f, 1.1f), 60);
            GeneralParticleHandler.SpawnParticle(wind);

            // Change the wind speeds periodically.
            if (attackTimer % 90f == 0f)
            {
                Main.windSpeedSet = Main.rand.NextFloat(-3f, 3f);
                Main.windSpeedTemp = MathHelper.Lerp(Main.windSpeedTemp, Main.windSpeedSet, 0.85f);
                Main.PlaySound(InfernumSoundRegistry.StormWeaverWindSound);

                // Create a gust of wind particles in the new direction.
                for (int i = 0; i < 85; i++)
                {
                    windSpawnPosition = target.Center + new Vector2(Main.windSpeedTemp.DirectionalSign() * -1250f, Main.rand.NextFloatDirection() * 900f);
                    windVelocity = new Vector2(1f, 0.25f) * Main.windSpeedTemp.DirectionalSign() * Main.rand.NextFloat(0.1f, 1.8f) * 90f;
                    wind = new SnowyIceParticle(windSpawnPosition, windVelocity, Color.White * 0.6f, Main.rand.NextFloat(1f, 2f), 60);
                    GeneralParticleHandler.SpawnParticle(wind);
                }
            }
            ...
            if (attackTimer <= windGustTime)
            {
                ...
                npc.velocity = npc.SafeDirectionTo(spinDestination) * (float)Math.Min(npc.Distance(spinDestination), 34f);
                ...
                if (attackTimer % windGustReleaseRate == 0f)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        {
                            ProjectileSpawnManagementSystem.PrepareProjectileForSpawning(windgust =>
                            {
                                windgust.ModProjectile<WindGust>().SpinDirection = spinDirection;
                                windgust.ModProjectile<WindGust>().SpinCenter = target.Center;
                            });
        ...
        public static void CreateSparks(Vector2 sparkSpawnPosition)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                GeneralParticleHandler.SpawnParticle(new SparkParticle(sparkSpawnPosition, sparkVelocity, 60, 2f, sparkColor));
