    ...
    public class GolemBodyBehaviorOverride : NPCBehaviorOverride
    {
        ...
        public override bool PreAI(NPC npc)
        {
            ...
            if (AITimer == 0f)
            {
                ...
                {
                    int freeHeadInt = NPC.NewNPC((int)npc.Center.X - 55, (int)npc.Top.Y, NPCID.GolemHeadFree, 0, npc.whoAmI);
                    ...
                    int attachedHeadInt = NPC.NewNPC((int)npc.Center.X - 55, (int)npc.Top.Y, NPCID.GolemHead, 0, npc.whoAmI);
                    ...
                    int leftHand = NPC.NewNPC((int)leftHandCenterPos.X, (int)leftHandCenterPos.Y, ModContent.NPCType<GolemFistLeft>(), 0, npc.whoAmI);
                    LeftFistNPC = leftHand;

                    int rightHand = NPC.NewNPC((int)rightHandCenterPos.X, (int)rightHandCenterPos.Y, ModContent.NPCType<GolemFistRight>(), 0, npc.whoAmI);
            ...
            if (FightStarted == 0f)
            {
                ...
                if (Timer == 120f)
                {
                    if (AttackState == (float)GolemAttackState.BadTime)
                        Main.PlaySound(InfernumSoundRegistry.GolemSansSound, target.Center);
                    else
                        Main.PlaySound(InfernumSoundRegistry.GolemSpamtonSound, target.Center);
                }
        ...
        public static void DoBehavior_FloorFire(NPC npc, Player target, bool inPhase2, bool inPhase3, ref float attackTimer, ref float attackCooldown, ref float jumpState, ref float attackCounter, ref float slamTelegraphInterpolant)
        {
            ...
            else if (jumpState == 1f)
            {
                ...
                if (attackTimer < maxHoverTime)
                {
                    ...
                    float hoverSpeed = Utilities.Remap(attackTimer, 0f, maxHoverTime, 43.5f, 108f);
                ...
                else if (attackTimer < maxHoverTime + slamDelay)
                {
                    ...
                    slamTelegraphInterpolant = Utils.InverseLerp(0f, slamDelay, attackTimer - maxHoverTime, true);
                }
                ...
                else
                {
                    ...
                    if (hitGround)
                    {
                        ...
                        Main.PlaySound(SoundID.Item14, npc.Bottom);
                        Main.PlaySound(InfernumSoundRegistry.GolemGroundHitSound, npc.Bottom);
                        ...
                        for (int i = 0; i < 20; i++)
                        {
                            ...
                            StoneDebrisParticle2 rock = new StoneDebrisParticle2(rockSpawnPosition, Vector2.UnitY * Main.rand.NextFloat(5f, 8f), Color.Brown, Main.rand.NextFloat(1f, 1.4f), 90);
            ...
            {
                Vector2 platformSpawnPosition = new Vector2(target.Center.X, npc.Infernum().Arena.Bottom - 16f);
        ...
        public static void DoBehavior_SpiketrapWaves(NPC npc, bool inPhase2, bool inPhase3, ref float attackTimer, ref float attackCooldown)
        {
            ...
            {
                fistSlamInterpolant = (float)Math.Pow(attackTimer / fistSlamTime, 2f);

                // Create impact effects.
                if (attackTimer == fistSlamTime - 1f)
                {
                    Vector2 leftImpactPoint = new Vector2(npc.Infernum().Arena.Left, leftFist.Center.Y);
                    Vector2 rightImpactPoint = new Vector2(npc.Infernum().Arena.Right, leftFist.Center.Y);
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    Dust.NewDustDirect(trapSpawnPosition, 8, 600, DustID.Fire);

                    trapSpawnPosition = npc.Infernum().Arena.BottomRight();
                    Dust.NewDustDirect(trapSpawnPosition - new Vector2(-8f, 600f), 8, 600, DustID.Fire);
                }
            }
            ...
            {
                Vector2 platformSpawnPosition = new Vector2(npc.Infernum().Arena.Left + 12f, npc.Infernum().Arena.Center.Y + 160f);
        ...
        public static void DoBehavior_HeatRay(NPC npc, Player target, bool inPhase2, bool inPhase3, bool headIsFree, ref float attackTimer, ref float attackCooldown, ref float eyeLaserRayInterpolant)
        {
            ...
            if (attackTimer < hoverTelegraphTime)
            {
                ...
                freeHead.velocity = (freeHead.velocity * 7f + freeHead.SafeDirectionTo(hoverDestination) * Math.Min(freeHead.Distance(hoverDestination), movementSpeed)) / 8f;
        ...
        public static void DoBehavior_SpinLaser(NPC npc, Player target, bool inPhase3, ref float attackTimer, ref float coreLaserRayInterpolant, ref float coreLaserRayDirection, ref float attackCooldown)
        {
            ...
            if (attackTimer % platformReleaseRate == 0f)
            {
                ...
                Vector2 platformSpawnOffset = new Vector2(0f, npc.Infernum().Arena.Height * MathHelper.Lerp(0.2f, 0.8f, platformVerticalInterpolant));
            ...
            if (attackTimer >= laserTelegraphTime)
                target.Calamity().GeneralScreenShakePower = Math.Max(target.Calamity().GeneralScreenShakePower, 2f);
            ...
            ref float slingshotRotation, ref float attackCounter, ref float attackCooldown)
        {
            ...
            {
                armSlamInterpolant = (float)Math.Pow(Utils.InverseLerp(0f, fistSlamTime, attackTimer - dustTelegraphTime, true), 2f);
            ...
            npc.Center = Vector2.Lerp(npc.Center, bodyDestination, (float)Math.Pow(bodySlamInterpolant, 8.4f));
        ...
        {
            Main.PlaySound(InfernumSoundRegistry.GolemGroundHitSound);
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                if (Main.npc[i].active && Main.npc[i].type == platformID)
                {
                    ...
                    StrongBloom fire = new StrongBloom(Main.npc[i].Center, Vector2.Zero, Color.Orange * 0.56f, 2f, 35);
                    GeneralParticleHandler.SpawnParticle(fire);

                    for (int j = 0; j < 8; j++)
                    {
                        Vector2 rockSpawnPosition = Main.npc[i].Center + Main.rand.NextVector2Circular(40f, 10f);
                        StoneDebrisParticle2 rock = new StoneDebrisParticle2(rockSpawnPosition, -Vector2.UnitY.RotatedByRandom(0.8f) * Main.rand.NextFloat(3f, 13f), Color.Brown, Main.rand.NextFloat(1f, 1.4f), 90);
                        GeneralParticleHandler.SpawnParticle(rock);
                    }
        ...
        public static Color PrimitiveTrailColor(NPC npc, float completionRatio)
        {
            ...
            float opacity = Utils.InverseLerp(0f, 0.4f, interpolant, true) * Utils.InverseLerp(1f, 0.8f, interpolant, true);
        ...
        public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            if (npc.Infernum().OptionalPrimitiveDrawer is null)
				npc.Infernum().OptionalPrimitiveDrawer = new PrimitiveTrailCopy(PrimitiveWidthFunction, c => PrimitiveTrailColor(npc, c), null, true, InfernumEffectsRegistry.SideStreakVertexShader);
