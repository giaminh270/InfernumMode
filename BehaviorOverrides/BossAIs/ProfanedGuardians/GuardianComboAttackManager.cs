    ...
    public static partial class GuardianComboAttackManager
    {
                ...
                if (attackTimer == 1)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        for (int i = 0; i < wallsToSimulate; i++)
                        {
                            ...
                            while (Vector2.Distance(newCenter, previousCenter) > 400f || attempts < 30);
        ...
        public static void DoBehavior_FlappyBird(NPC npc, Player target, ref float attackTimer, NPC commander)
        {
            ...
            if (npc.type == CommanderType)
            {
                ...
                {
                    float sine = (float)Math.Sin(movementTimer * 0.05f);
                ...
                if (Main.npc.IndexInRange(GlobalNPCOverrides.ProfanedCrystal))
                {
                    if (Main.npc[GlobalNPCOverrides.ProfanedCrystal].active)
                    {
                        ...
                        else
                        {
                            npc.velocity.X = 0f;
                            movedToPosition = 1f;
                            float sine = (float)-Math.Sin(Main.GlobalTime * 2f);
                            npc.velocity.Y = sine * 1.5f;
                            npc.spriteDirection = -1;
                        }
            ...
            else if (npc.type == HealerType)
            {
                ...
                {
                    spawnedCrystal = 1;
                    int npcIndex = NPC.NewNPC((int)CrystalPosition.X, (int)CrystalPosition.Y, ModContent.NPCType<HealerShieldCrystal>(), 0, 0, 0, 0, 0, target.whoAmI);
        ...
        public static void DoBehavior_SoloHealer(NPC npc, Player target, ref float universalAttackTimer, NPC commander)
        {
            ...
            if (npc.type == CommanderType)
            {
                ...
                if (npc.Distance(hoverPosition) > 15f && movedToPosition == 0f)
                {
                    ...
                    npc.velocity = (npc.velocity * 7f + npc.SafeDirectionTo(hoverPosition) * Math.Min(npc.Distance(hoverPosition), 18)) / 8f;
                }
                else
                {
            ...
            else if (npc.type == DefenderType)
            {
                ...
                npc.spriteDirection = Math.Sign(npc.DirectionTo(target.Center).X);

                //float xOffset = 500f * -MathF.Sign(npc.DirectionTo(target.Center).X);
                Vector2 hoverPosition = target.Center + new Vector2(500f, -350);
                npc.velocity = (npc.velocity * 7f + npc.SafeDirectionTo(hoverPosition) * Math.Min(npc.Distance(hoverPosition), flySpeed)) / 8f;
            ...
            else if (npc.type == HealerType)
            {
                ...
                if (npc.Distance(hoverPosition) > 5f && movedToPosition == 0f)
                    npc.velocity = (npc.velocity * 5f + npc.SafeDirectionTo(hoverPosition) * Math.Min(npc.Distance(hoverPosition), 25)) / 8f;
                else
                {
                    drawShieldConnections = 1f;
                    npc.velocity.X *= 0.9f;
        ...
        public static void DoBehavior_SoloDefender(NPC npc, Player target, ref float universalAttackTimer, NPC commander)
        {
            ...
            else if (npc.type == DefenderType)
            {
                ...
                Vector2 oldHoverOffset = new Vector2(hoverOffsetX, hoverOffsetY);
                ...
                switch (substate)
                {
                    ...
                    case 0:
                        ...
                        for (int i = 0; i < 100; i++)
                        {
                            ...
                            if (angleBetween < (MathHelper.PiOver2 + 0.01f) && angleBetween != 0)
                                break;
                        }
                    ...
                    case 1:
                        ...
                        npc.velocity = (npc.velocity * 7f + npc.SafeDirectionTo(position) * Math.Min(npc.Distance(position), 33f)) / 8f;
            ...
            else if (npc.type == HealerType)
            {
                ...
                npc.spriteDirection = Math.Sign(npc.DirectionTo(commander.Center).X);
        ...
        public static void DoBehavior_HealerAndDefender(NPC npc, Player target, ref float universalAttackTimer, NPC commander)
        {
            ...
            else if (npc.type == HealerType)
            {
                ...
                Vector2 hoverPosition = new Vector2(commander.Center.X, CrystalPosition.Y - 550);
                ...
                if (universalAttackTimer % crystalLayerFireRate == crystalLayerFireRate - 1 && Main.netMode != NetmodeID.MultiplayerClient && completedCrystalLayers < totalCrystalLayers)
                {
                    ...
                    {
                        Vector2 shootVelocity = new Vector2(MathHelper.Lerp(-20f, 20f, i / (float)totalCrystalsPerLayer), -10.75f);
        ...
        public static void DoBehavior_HealerDeathAnimation(NPC npc, Player target, ref float universalAttackTimer, NPC commander)
        {
            ...
            if (npc.type == CommanderType)
            {
                ...
                {
                    float interlopant = (float)Math.Sin(Math.PI * ((universalAttackTimer - angerDelay) / angerTime));
                    glowAmount = Utilities.SineInOutEasing(interlopant, 0);
                }
            ...
            else if (npc.type == HealerType)
            {
                ...
                if (universalAttackTimer <= whiteGlowTime)
                    whiteGlowOpacity = Utilities.ExpInEasing(MathHelper.Lerp(0f, 1f, universalAttackTimer / whiteGlowTime), 0);
        ...
        public static void DoBehavior_SpearDashAndGroundSlam(NPC npc, Player target, ref float universalAttackTimer, NPC commander)
        {
            ...
            if (npc.type == CommanderType)
            {
                ...
                switch (substate)
                {
                    ...
                    case 1:
                        xOffset = -700f * Math.Sign(target.Center.X - npc.Center.X);
                        Vector2 hoverDestination = target.Center + new Vector2(xOffset, 0f);
                        npc.velocity = (npc.velocity * 7f + npc.SafeDirectionTo(hoverDestination) * Math.Min(npc.Distance(hoverDestination), flySpeed)) / 8f;
                    ...
                    case 2:
                        ...
                        if (localAttackTimer == 0)
                            Main.PlaySound(InfernumSoundRegistry.MyrindaelSpinSound, target.Center);

                        if (localAttackTimer < spinLength)
                        {
                            spearRotation = (float)(2 * Math.PI) * Utilities.SineInOutEasing(localAttackTimer / spinLength, 0) + spearRotationStartingOffset;
                            if (localAttackTimer % spearReleasePoint == spearReleasePoint - 1f && Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                float positionRotation = (float)(2 * Math.PI) * Utilities.LinearEasing(localAttackTimer / spinLength, 0) + spearRotationStartingOffset;
                ...
                switch (substate)
                {
                    ...
                    case 4:
                        ...
                        if ((Collision.SolidCollision(npc.Center, (int)(npc.width * 0.85f), (int)(npc.height * 0.85f)) && npc.Center.Y > target.Center.Y) || universalAttackTimer >= slamLength)
                        {
                            ...
                            Vector2 start = new Vector2(npc.Center.X - 3000f, yPos);
                            Vector2 end = new Vector2(npc.Center.X + 3000f, yPos);
                            ...
                            for (int j = 0; j < (InfernumConfig.Instance.ReducedGraphicsConfig ? 20 : 40); j++)
                            {
                    ...
                    case 5:
                        ...
                        if (universalAttackTimer > afterSlamWaitLength)
                        {
                            ...
                            if (slamsPerformed >= maxSlams)
                            {
                                ...
                                if ((commander.Infernum().ExtraAI[0] == 0f || commander.Infernum().ExtraAI[0] == 1f) && pillarsAreMostlyGone)
                                    SelectNewAttack(commander, ref universalAttackTimer);
                            }
                            else
                            {
        ...
        public static void DoBehavior_CrashRam(NPC npc, Player target, ref float universalAttackTimer, NPC commander)
        {
            ...
            if (npc.type == CommanderType)
            {
                ...
                switch (substate)
                {
                    ...
                    case 3:
                        ...
                        for (int i = 0; i < (InfernumConfig.Instance.ReducedGraphicsConfig ? 10 : 30); i++)
                        {
                            // Bias towards lower values. 
                            float size = (float)Math.Pow(Main.rand.NextFloat(), 2f);
                            InfernumFusableParticleManager.GetParticleSetByType<ProfanedLavaParticleSet>()?.SpawnParticle(npc.Center - (npc.velocity * 0.5f) + (Main.rand.NextVector2Circular(npc.width * 0.5f, npc.height * 0.5f) * size),
                    ...
                    case 4:
                        if (universalAttackTimer == 1f)
                        {
                            ...
                            for (int i = 0; i < 15; i++)
                            {
                                Particle rock = new ProfanedRockParticle(impactCenter, -Vector2.UnitY.RotatedByRandom((float)(2 * Math.PI)) * Main.rand.NextFloat(6f, 9f),
            ...
            else if (npc.type == DefenderType)
            {
                ...
                switch (substate)
                {
                    ...
                    // Move to said offset.
                    case 1:
                        Vector2 hoverDestination = target.Center + ((float)(commanderOffset + Math.PI)).ToRotationVector2() * defenderHoverDistance;
            ...
            if (npc.type == CommanderType)
            {
                ...
                if (universalAttackTimer <= fireballReleaseRate && substate == 2)
                {
                    ...
                    if (universalAttackTimer < fireballReleaseRate)
                    {
                        ...
                        SquishyLightParticle light = new SquishyLightParticle(lightSpawnPosition, lightVelocity, scale, lightColor, lightLifetime, 1f, squishFactor, squishFactor * 4f);
                        GeneralParticleHandler.SpawnParticle(light);

                        // Create pulse rungs and bloom periodically.
                        if (universalAttackTimer % 15f == 0f)
                        {
                            PulseRing ring = new PulseRing(npc.Center, Vector2.Zero, lightColor, 3.6f, 0f, 60);
                            GeneralParticleHandler.SpawnParticle(ring);

                            StrongBloom bloom = new StrongBloom(npc.Center, Vector2.Zero, lightColor, 1f, 15);
                            GeneralParticleHandler.SpawnParticle(bloom);
                        }

                        // Create energy sparks at the center.
                        CritSpark spark = new CritSpark(npc.Center, Main.rand.NextVector2Circular(8f, 8f), Color.OrangeRed, Color.Gold, 5f, 6, 0.01f, 7.5f);
                    ...
                    else if (substate == 2)
                    {
                        ...
                        for (int i = 0; i < lightAmount; i++)
                        {
                            ...
                            if (i == 0)
                            {
                                PulseRing ring = new PulseRing(npc.Center, Vector2.Zero, lightColor, 0f, 3.6f, 30);
                                GeneralParticleHandler.SpawnParticle(ring);
                ...
                switch (substate)
                {
                    ...
                    case 1:
                        // Don't deal damage while moving.
                        npc.damage = 0;
                        Vector2 hoverDestination = new Vector2(xPosToMoveTo, CenterOfGarden.Y);
                    ...
                    case 2:
                        ...
                        if (universalAttackTimer % fireballReleaseRate == fireballReleaseRate - 1f && fireballRingsReleased < maxFireballRingsToRelease)
                        {
                            ...
                            for (int i = 0; i < (InfernumConfig.Instance.ReducedGraphicsConfig ? 10 : 30); i++)
                            {
                                ...
                                CloudParticle fireExplosion = new CloudParticle(position, npc.Center.DirectionTo(position) * Main.rand.NextFloat(1f, 2f),
        ...
        public static void DoBehavior_DefenderDeathAnimation(NPC npc, Player target, ref float universalAttackTimer, NPC commander)
        {
            ...
            if ((rightHandIndex == 0 && leftHandIndex == 0))
            {
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    leftHandIndex = NPC.NewNPC((int)npc.Center.X - 160, (int)npc.Center.Y, ModContent.NPCType<EtherealHand>(), 0, -1);
                    rightHandIndex = NPC.NewNPC((int)npc.Center.X + 160, (int)npc.Center.Y, ModContent.NPCType<EtherealHand>(), 0, 1);
                    npc.netUpdate = true;
                }
            }
            ...
            if (npc.type == CommanderType)
            {
                ...
                target.Infernum().ScreenFocusInterpolant = 1f;
                target.Infernum().ScreenFocusPosition = focusPosition;
                if (substate < 6f)
                {
            ...
            else if (npc.type == DefenderType)
            {
                ...
                Vector2 originalRightHandPos = new Vector2(catchingRightHandXOG, catchingRightHandYOG);
                Vector2 originalLeftHandPos = new Vector2(catchingLeftHandXOG, catchingLeftHandYOG);
                float flySpeed = 20f;
                ...
                switch (substate)
                {
                    ...
                    case 0:
                        ...
                        if (localAttackTimer == 0)
                            typeof(MoonlordDeathDrama).GetField("whitening", Utilities.UniversalBindingFlags).SetValue(null, 1f);//MoonlordDeathDrama.RequestLight(1f/*1f - Utils.InverseLerp(15f, 45f, universalAttackTimer, true)*/, target.Center);
                    ...
                    case 3:
                        ...
                        RightHandPosition = new Vector2(-150f, -75f);
                        LeftHandPosition = new Vector2(-40f, 25f);
                    ...
                    case 4:
                        ...
                        float leftHandMoveInterpolant = Utilities.SineInOutEasing(localAttackTimer / reelbackTime, 0);
                        moveHands = 1f;
                    ...
                    case 5:
                        rightHandMoveInterpolant = Utilities.ExpInEasing(localAttackTimer / launchTime, 0);
                        leftHandMoveInterpolant = Utilities.SineInOutEasing(localAttackTimer / launchTime, 0);
                        moveHands = 1f;
            ...
            switch (substate)
            {
                ...
                case 0:
                    ...
                    if (npc.WithinRange(hoverDestination, 100f))
                    {
                        ...
                        for (int i = 0; i < 4; i++)
                        {
                            Vector2 position = new Vector2(npc.Center.X + Main.rand.NextFloat(-250f, 250f), Main.screenHeight + -100f + Main.screenPosition.Y + Main.rand.NextFloat(-70f, 70f));
                ...
                case 1:
                    ...
                    else if (universalAttackTimer == recoilDownwardsTime)
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            Vector2 center = new Vector2(npc.Center.X, (Main.maxTilesY * 16f) - 50f);
                    ...
                    {
                        Main.PlaySound(InfernumSoundRegistry.VassalJumpSound, target.Center);
                        float lerpValue = MathHelper.Clamp(Utils.InverseLerp(300, 500, npc.Center.Y - target.Center.Y, false), 0f, 0.35f);
                ...
                case 3:
                    ...
                    for (int i = 0; i < (InfernumConfig.Instance.ReducedGraphicsConfig ? 25 : 50); i++)
                    {
                        // Bias towards lower values. 
                        float size = (float)Math.Pow(Main.rand.NextFloat(), 2f);
                        InfernumFusableParticleManager.GetParticleSetByType<ProfanedLavaParticleSet>()?.SpawnParticle(npc.Center - (npc.velocity * 0.5f) + (Main.rand.NextVector2Circular(npc.width * 0.5f, npc.height * 0.5f) * size),
                    ...
                    if ((Collision.SolidCollision(npc.Center, (int)(npc.width * 0.85f), (int)(npc.height * 0.85f)) && npc.Center.Y > target.Center.Y) || universalAttackTimer >= maxChargeLength)
                    {
                        ...
                        for (int i = 0; i < (InfernumConfig.Instance.ReducedGraphicsConfig ? 4 : 20); i++)
                        {
                        ...
                        for (int i = 0; i < 10; i++)
                        {
                            MediumMistParticle fireExplosion = new MediumMistParticle(impactCenter + Main.rand.NextVector2Circular(80f, 80f), Vector2.Zero,
                                Main.rand.NextBool() ? WayfinderSymbol.Colors[0] : WayfinderSymbol.Colors[1],
                        ...
                        for (int i = 0; i < 70; i++)
                            InfernumFusableParticleManager.GetParticleSetByType<ProfanedLavaParticleSet>()?.SpawnParticle(npc.Center + Main.rand.NextVector2Circular(150f, 150f), Main.rand.NextFloat(52f, 85f));
        ...
        public static void DoBehavior_DogmaLaserBall(NPC npc, Player target, ref float universalAttackTimer)
        {
            ...
            switch (substate)
            {
                ...
                case 2:
                    ...
                    {
                        float chargeUpInterpolant = Utils.InverseLerp(0f, chargeUpTime, universalAttackTimer, true);
                        for (int i = 0; i < 2; i++)
                        {
            ...
            switch (substate)
            {
                ...
                case 1:
                    ...
                    if (universalAttackTimer % shootDelay >= shootDelay * 0.60f)
                    {
                        ...
                        SparkParticle laserEnergy = new SparkParticle(energySpawnPosition, energyVelocity, 15, 1f, energyColor);
                        GeneralParticleHandler.SpawnParticle(laserEnergy);
                    }

                    if (universalAttackTimer % shootDelay == 0f && universalAttackTimer <= totalShootLength)
                    {
                        Main.PlaySound(InfernumSoundRegistry.ProvidenceSpearHitSound, target.Center);
                        Main.PlaySound(SoundID.DD2_LightningBugZap, target.Center);

                        ScreenEffectSystem.SetFlashEffect(spearTip, 0.5f, 45);

                        if (!CalamityConfig.Instance.DisableScreenShakes)
                            target.Infernum().CurrentScreenShakePower = 4f;
                        ...
                        for (int i = 0; i < 4; i++)
                        {
                            ...
                            GeneralParticleHandler.SpawnParticle(new SparkParticle(spearTip, sparkVelocity, 60, 2f, sparkColor));
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            {
                                s.ModProjectile<HolySineSpear>().WaveOffset = Main.rand.NextFloat(0f, (float)(2 * Math.PI));
                            });
        ...
        public static void DoBehavior_SpearSpinThrow(NPC npc, Player target, ref float universalAttackTimer)
        {
            ...
            switch (substate)
            {
                ...
                case 0:
                    ...
                    hoverXOffset = 400f * -Math.Sign(npc.SafeDirectionTo(target.Center).X);
                ...
                case 2:
                    ...
                    {
                        spearRotation = angleToStab * Utilities.SineInOutEasing(universalAttackTimer / spearSpinTime, 0) + spearRotationStartingOffset;
                    }
                    else if (universalAttackTimer <= spearReelbackTime)
                    {
                        ...
                        spearPosOffset = MathHelper.Lerp(0f, -50f, Utilities.SineInOutEasing(interpolant, 0));
                    }
                    else if (universalAttackTimer <= spearStabTime)
                    {
                        ...
                        spearPosOffset = MathHelper.Lerp(-50f, 30f, Utilities.SineInOutEasing(interpolant, 0));
                    }
                    else
                    {
                        Vector2 spearTip = npc.Center + spearRotation.ToRotationVector2() * (120f * 1.5f);
                        for (int i = 0; i < 100; i++)
                            InfernumFusableParticleManager.GetParticleSetByType<ProfanedLavaParticleSet>()?.SpawnParticle(spearTip + Main.rand.NextVector2Circular(50f, 50f), Main.rand.NextFloat(52f, 85f));
                ...
                case 3:
                    if (universalAttackTimer == 1)
                    {
                        ...
                        for (int i = 0; i < 100; i++)
                            InfernumFusableParticleManager.GetParticleSetByType<ProfanedLavaParticleSet>()?.SpawnParticle(spawnPosition + Main.rand.NextVector2Circular(75f, 75f), Main.rand.NextFloat(52f, 85f));
        ...
        public static void DoPhaseTransitionEffects(NPC commander, int phaseTransition)
        {
            ...
            commander.Infernum().ExtraAI[GuardianSkyExtraIntensityIndex] = extraAIValue;

            LegacySoundStyle soundToPlay;
            switch (phaseTransition)
            {
                case 1:
                    soundToPlay = InfernumSoundRegistry.GuardianSpawnSound;
                    break;
                case 2:
                    soundToPlay = InfernumSoundRegistry.GuardiansPhaseTwoTransition;
                    break;
                default:
                    soundToPlay = InfernumSoundRegistry.GuardiansPhaseThreeTransition;
                    break;
            }
            Main.PlaySound(soundToPlay, commander.Center);

            if (Main.player.IndexInRange(commander.target))
            {
                Main.player[commander.target].Infernum().CurrentScreenShakePower = commander.Infernum().ExtraAI[GuardianSkyExtraIntensityIndex] * 12f;
        ...
        public static void SelectNewAttack(NPC commander, ref float universalAttackTimer, float specificAttackToSwapTo = -1)
        {
            ...
            if (Main.npc.IndexInRange(CalamityGlobalNPC.doughnutBossDefender))
            {
                if (Main.npc[CalamityGlobalNPC.doughnutBossDefender].active)
                {
                    ...
                    {
                        if ((DefenderShieldStatus)defender.Infernum().ExtraAI[DefenderShieldStatusIndex] == DefenderShieldStatus.ActiveAndAiming ||
                        (DefenderShieldStatus)defender.Infernum().ExtraAI[DefenderShieldStatusIndex] == DefenderShieldStatus.ActiveAndStatic)
