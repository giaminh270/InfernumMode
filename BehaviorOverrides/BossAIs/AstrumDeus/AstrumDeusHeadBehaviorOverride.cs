...
namespace InfernumMode.BehaviorOverrides.BossAIs.AstrumDeus
{
    public class AstrumDeusHeadBehaviorOverride : NPCBehaviorOverride
    {
        ...
        public override bool PreAI(NPC npc)
        {
            ...
            if (beacons.Count == 0)
            {
                Projectile.NewProjectile(target.Center, Vector2.Zero, ModContent.ProjectileType<DeusRitualDrama>(), 0, 0f);
            ...
            float beaconAngerFactor = Utils.InverseLerp(EnrageStartDistance, EnrageStartDistance + 800f, MathHelper.Distance(beacons.First().Center.X, target.Center.X), true);
            ...
            {
                CreateSegments(npc, 65, ModContent.NPCType<AstrumDeusBodySpectral>(), ModContent.NPCType<AstrumDeusTailSpectral>());
        ...
        public static void DoBehavior_VortexLemniscate(NPC npc, Player target, bool phase2, bool phase3, ref float attackTimer)
        {
            ...
            if (hasCharged == 0f)
            {
                ...
                float sin2T = (float)Math.Sin(t * 2f);
            ...
            {
                Vector2 lemniscateCenter = new Vector2(lemniscateCenterX, lemniscateCenterY);
                Vector2[] lemniscateFoci = new Vector2[]
                {
                ...
                bool cyan = true;
                List<int> vortices = new List<int>();
        ...
        public static void DoBehavior_PlasmaAndCrystals(NPC npc, Player target, bool phase2, bool phase3, float beaconAngerFactor, ref float attackTimer)
        {
            ...
            if (attackTimer < shootTime)
            {
                ...
                if (attackTimer % crystalShootRate == crystalShootRate - 1f && crystalShootCandidates.Count >= 1)
                {
                    ...
                    Main.PlaySound(SoundID.Item27, bodyToShootFrom.Center);
            ...
            if (attackTimer == 2f)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    for (int i = 0; i < planetCount; i++)
                    {
                        ...
                        NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, deusSpawnID, npc.whoAmI, MathHelper.TwoPi * i / planetCount, offsetRadius, orbitalAngularVelocity);
                    }
                }
            }
        ...
        public static void DoBehavior_InfectedStarWeave(NPC npc, Player target, bool phase2, bool phase3, float beaconAngerFactor, ref float attackTimer)
        {
            ...
            {
                Vector2 startCenter = new Vector2(starSpawnCenterX, starSpawnCenterY);
                Main.PlaySound(SoundID.Item28, startCenter);
        ...
        {
            Texture2D texture = ModContent.GetTexture("CalamityMod/NPCs/AstrumDeus/AstrumDeusHeadSpectral");
            if (npc.Infernum().ExtraAI[7] == 1f)
            {
                texture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/AstrumDeus/AstrumDeusHeadExposed");
