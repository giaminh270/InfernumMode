...
namespace InfernumMode.BehaviorOverrides.BossAIs.AstrumDeus
{
    public class AstrumDeusTailBehaviorOverride : NPCBehaviorOverride
    {
        ...
        {
            Texture2D texture = ModContent.GetTexture("CalamityMod/NPCs/AstrumDeus/AstrumDeusTailSpectral");
            if (Main.npc[npc.realLife].Infernum().ExtraAI[7] == 1f)
            {
                texture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/AstrumDeus/AstrumDeusTailExposed");
