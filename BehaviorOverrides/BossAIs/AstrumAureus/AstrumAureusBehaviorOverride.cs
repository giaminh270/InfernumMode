...
namespace InfernumMode.BehaviorOverrides.BossAIs.AstrumAureus
{
    public class AstrumAureusBehaviorOverride : NPCBehaviorOverride
    {
            ...
            switch ((int)attackState)
            {
                ...
                case 1:
                    ...
                    if (attackTimer < maxHoverTime)
                    {
                        ...
                        float hoverSpeed = Utilities.Remap(attackTimer, 0f, maxHoverTime, 38f, 108f);
        ...
        public static void DoAttack_AstralDrillLaser(NPC npc, Player target, float lifeRatio, ref float attackTimer, ref float frameType)
        {
            ...
            telegraphInterpolant = Utils.InverseLerp(0f, laserShootDelay, attackTimer, true);
        ...
        public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Color lightColor)
        {
            if (npc.Infernum().OptionalPrimitiveDrawer is null)
			 	npc.Infernum().OptionalPrimitiveDrawer = new PrimitiveTrailCopy(PrimitiveWidthFunction, c => PrimitiveTrailColor(npc, c), null, true, GameShaders.Misc["Infernum:SideStreak"]);
            ...
            if (currentAttack != AureusAttackType.Recharge && glowmaskTexture != Main.npcTexture[npc.type])
            {
