...
namespace InfernumMode.BehaviorOverrides.BossAIs.Draedon.ComboAttacks
{
    public static partial class ExoMechComboAttackContent
    {
        public static bool UseTwinsThanatosComboAttack(NPC npc, float twinsHoverSide, ref float attackTimer, ref float frameType)
        {
			...
			ExoMechComboAttackType currentAttack = (ExoMechComboAttackType)initialMech.ai[0];
			bool result = false;
			switch (currentAttack)
            {
			    case ExoMechComboAttackType.TwinsThanatos_ThermoplasmaDashes:
			        result = DoBehavior_TwinsThanatos_ThermoplasmaDashes(npc, target, ref attackTimer, ref frameType);
			        break;
			    case ExoMechComboAttackType.TwinsThanatos_AlternatingTwinsBursts:
			        result = DoBehavior_TwinsThanatos_AlternatingTwinsBursts(npc, target, ref attackTimer, ref frameType);
			        break;
			    default:
			        result = false;
			        break;
        ...
        public static bool DoBehavior_TwinsThanatos_ThermoplasmaDashes(NPC npc, Player target, ref float attackTimer, ref float frameType)
        {
            ...
            if (attackTimer <= 60f)
                thanatosFlySpeedFactor *= Utils.InverseLerp(-15f, 60f, attackTimer, true);
            ...
            if (ExoTwinsAreInSecondPhase)
            {
                ...
                bool twinsInSecondPhase = CurrentTwinsPhase != 4 && CurrentTwinsPhase != 0;
        ...
        public static bool DoBehavior_TwinsThanatos_AlternatingTwinsBursts(NPC npc, Player target, ref float attackTimer, ref float frameType)
        {
            ...
            if (npc.type == ModContent.NPCType<ThanatosHead>())
            {
                ...
                {
                    float flySpeed = Utilities.Remap(npc.Distance(target.Center), 750f, 2700f, 6f, 24f);
                    float aimInterpolant = Utilities.Remap(wrappedAttackTimer, 0f, redirectTime - 4f, 0.01f, 0.5f);
            ...
            if (isExoTwin)
            {
                ...
                if (isAttackingExoTwin)
                {
                    ...
                    if (npc.type == ModContent.NPCType<Artemis>())
                    {
                        ...
                        {
                            Main.PlaySound(InfernumSoundRegistry.ArtemisSpinLaserbeamSound);
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                for (int i = 0; i < 2; i++)
                                {
                        ...
                        {
                            float slowdownInterpolant = Utils.InverseLerp(artemisHoverTime - 40f, artemisHoverTime - 24f, localAttackTimer, true);
                    ...
                    if (npc.type == ModContent.NPCType<Apollo>())
                    {
                        ...
                        if (localAttackTimer <= apolloTelegraphDelay)
                        {
                            ...
                            float slowdownInterpolant = Utils.InverseLerp(apolloTelegraphDelay - 50f, apolloTelegraphDelay - 36f, localAttackTimer, true);
                            ...
                            if (localAttackTimer == apolloTelegraphDelay - 36f)
                            {
                                ...
                                if (Main.netMode != NetmodeID.MultiplayerClient)
                                {
                                    ...
                                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ModContent.ProjectileType<PlasmaChargeTelegraph>(), 0, 0f, npc.target, 0f, npc.whoAmI);
