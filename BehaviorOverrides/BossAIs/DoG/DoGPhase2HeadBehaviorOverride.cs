    ...
    public static class DoGPhase2HeadBehaviorOverride
    {
            ...
            if (phase2IntroductionAnimationTimer < DoGPhase2IntroPortalGate.Phase2AnimationTime)
            {
                ...
                if (!Utilities.AnyProjectiles(ModContent.ProjectileType<DoGPhase2IntroPortalGate>()))
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                        Projectile.NewProjectile(npc.Center, Vector2.Zero, ModContent.ProjectileType<DoGPhase2IntroPortalGate>(), 0, 0f);
                }
            ...
            if (canPerformSpecialAttacks)
            {
                ...
                if (performingSpecialAttack == 0f)
                {
                    ...
                    if (specialAttackTimer >= SpecialAttackDelay - SpecialAttackTransitionPreparationTime)
                    {
                        ...
                        {
                            Main.PlaySound(InfernumSoundRegistry.DoGLaughSound.WithVolume(3f), target.Center);
        ...
        public static void DoDeathEffects(NPC npc, float deathAnimationTimer, ref float destroyedSegmentsCount)
        {
            ...
            if (deathAnimationTimer == 1f)
				Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/DevourerSpawn"), Main.player[npc.target].Center);
            ...
            void DestroySegment(int index, ref float destroyedSegments)
            {
				...
				if (Main.rand.NextBool(5))
					Main.PlaySound(InfernumSoundRegistry.DevourerSegmentBreak, npc.Center);
            ...
            float idealSpeed = MathHelper.Lerp(9f, 4.75f, Utils.InverseLerp(15f, 210f, deathAnimationTimer, true));
            ...
            {
                int segmentToDestroy = (int)(Utils.InverseLerp(0f, segmentDestructionTime, deathAnimationTimer - textDelay, true) * 60f);
            ...
            if (deathAnimationTimer == deathAnimationTimer + segmentDestructionTime + 42f)
            {
                ...
                {
                    Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/DevourerSpawn").WithVolume(1.6f), npc.Center);

                    for (int i = 0; i < 3; i++)
                    {
                        float pitch = -MathHelper.Lerp(0.1f, 0.4f, i / 3f);
                        Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/TeslaCannonFire").WithVolume(1.8f).WithPitchVariance(pitch), npc.Center);
                    }
                }
            }
            ...
            {
                int segmentToDestroy = (int)(Utils.InverseLerp(0f, lastSegmentsDestructionTime, finalSegmentDestructionTimer, true) * 10f) + 60;
                DestroySegment(segmentToDestroy, ref destroyedSegmentsCount);
            }

            // Cover the screen in a white light before DoG's head explodes.
            float light = Utils.InverseLerp(20f, 55f, finalSegmentDestructionTimer, true);
        ...
        public static void DoAggressiveFlyMovement(NPC npc, Player target, bool dontChompYet, bool chomping, ref float jawRotation, ref float chompEffectsCountdown, ref float universalFightTimer, ref float flyAcceleration)
        {
            ...
            float swimOffsetAngle = (float)Math.Sin(MathHelper.TwoPi * universalFightTimer / 160f) * Utils.InverseLerp(400f, 540f, distanceFromBaseDestination, true) * 0.41f;
        ...
        public static void DoSpecialAttack_ChargeGates(NPC npc, Player target, bool finalPhase, ref float attackTimer, ref float segmentFadeType)
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient && wrappedAttackTimer == 20f)
            {
                ...
                Projectile.NewProjectile(portalSpawnPosition, Vector2.Zero, ModContent.ProjectileType<DoGChargeGate>(), 0, 0f, Main.myPlayer, 0f, portalTelegraphTime);
                npc.netUpdate = true;
            }
            ...
            if (wrappedAttackTimer == portalTelegraphTime + 20f)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    for (int i = 0; i < fireballCount; i++)
                    {
                        ...
                        Utilities.NewProjectileBetter(npc.Center + flameShootVelocity * 3f, flameShootVelocity, ModContent.ProjectileType<HomingDoGBurst>(), AcceleratingFireballDamage, 0f);
                ...
                Main.PlaySound(InfernumSoundRegistry.DoGAttack, target.Center);
            }
        ...
        public static void PerformPerpendicularPortalAttack(NPC npc, Player target, ref float segmentFadeType, ref float perpendicularPortalAttackTimer, ref float perpendicularPortalAngle, ref float damageImmunityCountdown)
        {
            ...
            switch (SurprisePortalAttackState)
            {
                ...
                case PerpendicularPortalAttackState.EnteringPortal:
                    ...
                    if (GeneralPortalIndex == -1)
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            Projectile.NewProjectile( portalSpawnPosition, Vector2.Zero, ModContent.ProjectileType<DoGChargeGate>(), 0, 0f);
                            npc.netUpdate = true;
                        }
                        return;
                    }
                ...
                case PerpendicularPortalAttackState.Waiting:
                    ...
                    if (perpendicularPortalAttackTimer <= 10f && perpendicularPortalAttackTimer >= 1f)
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
