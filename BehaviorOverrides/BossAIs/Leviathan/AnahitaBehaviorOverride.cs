...
namespace InfernumMode.BehaviorOverrides.BossAIs.Leviathan
{
    public class AnahitaBehaviorOverride : NPCBehaviorOverride
    {
        ...
        public static void DoBehavior_CreateWaterIllusions(NPC npc, Player target, bool enraged, ref float attackTimer, ref float horizontalAfterimageInterpolant)
        {
            ...
            float slowdownInterpolant = Utils.InverseLerp(-42f, 0f, attackTimer - fadeOutTime - shootTime, true);
            ...
            if (attackTimer == 1f)
                Main.PlaySound(SoundID.Item, target.Center, 165);

            // Fade out before teleporting above the target and creating water illusions.
            if (attackTimer <= fadeOutTime)
            {
                horizontalAfterimageInterpolant = Utils.InverseLerp(0f, fadeOutTime * 0.8f, attackTimer, true);
            ...
            if (adjustedAttackTimer >= 90f && adjustedAttackTimer % shootRate == shootRate - 1f && !npc.WithinRange(target.Center, 250f))
            {
                ...
                List<Entity> entitiesToShoot = new List<Entity>() { npc };
        ...
        public static void DoBehavior_PlaySinusoidalSong(NPC npc, Player target, bool enraged, Vector2 headPosition, ref float attackTimer)
        {
            ...
            {
				Main.harpNote = Main.rand.NextFloatDirection() * 0.25f;
                Main.PlaySound(SoundID.Item26, target.position);
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
        ...
        public static void DoBehavior_IceMistBarrages(NPC npc, Player target, bool enraged, Vector2 headPosition, ref float attackTimer)
        {
            ...
            {
                teleportTimer++;
                npc.Opacity = Utils.InverseLerp(teleportChargeTime - 1f, teleportChargeTime - 24f, teleportTimer, true);
            ...
            Vector2 teleportOffset = new Vector2((teleportCounter % 2f == 0f).ToDirectionInt() * horizontalTeleportOffset, verticalTeleportOffset);
        ...
        public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            if (horizontalAfterimageInterpolant > 0f)
            {
                for (int i = -3; i <= 3; i++)
                {
                    ...
                    Color afterimageColor = npc.GetAlpha(Color.Cyan) * (1f - horizontalAfterimageInterpolant);
