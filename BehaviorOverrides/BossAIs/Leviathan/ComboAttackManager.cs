        ...
        public static void DoBehavior_ExoTwinsBasicShotsPrecursor(NPC npc, Player target, ref float attackTimer)
        {
            ...
            if (canShoot)
            {
                ...
                if (npc.type == ModContent.NPCType<Siren>())
                {
                    ...
                    Main.PlaySound(SoundID.Item60, headPosition);
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                ...
                else
                {
                    ...
                    Main.PlaySound(SoundID.Item73, mouthPosition);
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
            ...
            if (npc.type == ModContent.NPCType<LeviathanNPC>())
            {
                ...
                if (wrappedAttackTimer < redirectTime)
                {
                    ...
                    if (wrappedAttackTimer == redirectTime / 2)
                        Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/LeviathanRoarCharge"), npc.Center);
                }
                ...
                {
                    Main.PlaySound(SoundID.Zombie, npc.Center, 54);
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
