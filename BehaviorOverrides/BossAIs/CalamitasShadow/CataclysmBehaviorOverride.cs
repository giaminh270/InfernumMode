...
namespace InfernumMode.BehaviorOverrides.BossAIs.CalamitasShadow
{
    public class CataclysmBehaviorOverride : NPCBehaviorOverride
    {
        ...
        public static AresCannonChargeParticleSet CataclysmEnergyDrawer
        {
            get;
            set;
        } = new AresCannonChargeParticleSet(-1, 12, 85f, Color.Red);

        public static AresCannonChargeParticleSet CatastropheEnergyDrawer
        {
            get;
            set;
        } = new AresCannonChargeParticleSet(-1, 12, 85f, Color.DeepSkyBlue);
        ...
        {
			npc.GivenName = CalamitasShadowBehaviorOverride.CustomNameCataclysm;
            DoAI(npc);
            return false;
        }
        ...
        {
            int cataclysmIndex = NPC.FindFirstNPC(ModContent.NPCType<CalamitasRun>());
            int catastropheIndex = NPC.FindFirstNPC(ModContent.NPCType<CalamitasRun2>());
            bool isCataclysm = npc.type == ModContent.NPCType<CalamitasRun>();
            bool isCatastrophe = npc.type == ModContent.NPCType<CalamitasRun2>();
        ...
        public static void DoBehavior_HorizontalCharges(NPC npc, Player target, bool isCataclysm, ref float attackTimer)
        {
            ...
            if (CalamityGlobalNPC.cataclysm == -1)
                CalamityGlobalNPC.cataclysm = NPC.FindFirstNPC(ModContent.NPCType<CalamitasRun>());
            if (CalamityGlobalNPC.catastrophe == -1)
                CalamityGlobalNPC.catastrophe = NPC.FindFirstNPC(ModContent.NPCType<CalamitasRun2>());
            ...
            switch ((int)attackState)
            {
                ...
                case 0:
                    ...
                    catastropheArmRotation = Utilities.Remap(attackTimer, 0f, 24f, 0f, -1.8f);
                ...
                case 1:
                    ...
                    catastropheArmRotation = Utilities.Remap(attackTimer, 0f, 9f, -1.8f, 1.1f) * Utils.InverseLerp(chargeTime + chargeSlowdownTime, chargeTime, attackTimer, true);
...
//Sus
                // Make catastrophe swing his blade.
				float anticipationEndHeight = 0f + (-2.4f);

				CurveSegment anticipation = new CurveSegment(EasingType.PolyOut, 0f, 0f, -2.4f, 4);
				CurveSegment slash = new CurveSegment(EasingType.PolyInOut, 0.34f, anticipationEndHeight, 5.8f, 3);

				float slashEndHeight = anticipationEndHeight + 5.8f;

				CurveSegment recovery = new CurveSegment(EasingType.PolyIn, 0.8f, slashEndHeight, -slashEndHeight, 8);

				float swingCompletion = Utils.InverseLerp(0f, fireReleaseRate, wrappedTimer % fireReleaseRate, true);
				catastropheArmRotation = PiecewiseAnimation(swingCompletion, new CurveSegment[] { anticipation, slash, recovery });
        ...
        public static void DoBehavior_BladeUppercutAndDashes(NPC npc, Player target, bool isCataclysm)
        {
            ...
            if (!isCataclysm)
            {
                ...
                if (wrappedAttackTimer <= attackDelay)
                {
                    ...
                    float flyInterpolant = Utilities.Remap(wrappedAttackTimer, 0f, attackDelay, 0.03f, 0.27f);
                    ...
                    float idealArmRotation = Utilities.Remap(wrappedAttackTimer, 0f, attackDelay, 0.1f, -1.76f);
        ...
        public static bool DrawBrother(NPC npc, SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            bool isCatastrophe = npc.type == ModContent.NPCType<CalamitasRun2>();
            Texture2D texture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/Cataclysm");
            if (isCatastrophe)
                texture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/Catastrophe");
            ...
            if (isCatastrophe)
            {
                texture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/CatastropheGlowmask");
                ...
                float armSquishFactor = 1f - Utils.InverseLerp(0.3f, 0.9f, npc.localAI[0], true) * Utils.InverseLerp(Pi - 0.3f, Pi - 0.9f, npc.localAI[0], true) * 0.33f;
                Texture2D armTexture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/CatastropheArm");
                Vector2 armTextureDrawPosition = mainDrawPosition + new Vector2(npc.spriteDirection * -12f, (float)Cos(armRotation) * -8f - 40f).RotatedBy(npc.rotation) * npc.scale;
            ...
            {
                texture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/CataclysmGlowmask");
                spriteBatch.Draw(texture, mainDrawPosition, npc.frame, npc.GetAlpha(Color.White), npc.rotation, origin, npc.scale, spriteEffects, 0f);
