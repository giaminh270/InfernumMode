...
namespace InfernumMode.BehaviorOverrides.BossAIs.CalamitasShadow
{
    public class CalamitasShadowBehaviorOverride : NPCBehaviorOverride
    {
        ...
        public static Dictionary<string, int> HexNameToProj => new Dictionary<string, int>()
        {
        ...
        public override bool PreAI(NPC npc)
        {
            ...
            bool anyBrothers = NPC.AnyNPCs(ModContent.NPCType<CalamitasRun>()) || NPC.AnyNPCs(ModContent.NPCType<CalamitasRun2>());
            ...
            npc.gfxOffY = 4f;
            npc.Calamity().DoesNotGenerateRage = false;
            ...
            if (hexApplicationPauseDelay >= 1f && !phase3)
            {
                ...
                {
                    List<string> hexesToSpawn = new List<string>();
            ...
            {
                Main.PlaySound(SoundID.NPCDeath39, target.Center);
        ...
        public static void DoBehavior_SpawnAnimation(NPC npc, Player target, ref float attackTimer, ref float backgroundEffectIntensity, ref float blackFormInterpolant, ref float eyeGleamInterpolant, ref float armRotation, ref float forcefieldScale, ref float frameVariant, ref float foughtInUnderworld)
        {
            ...
            InfernumMode.BlackFade = Utils.InverseLerp(0f, blackFadeoutTime, attackTimer, true) * Utils.InverseLerp(blackFadeoutTime + blackFadeinTime + maximumDarknessTime, blackFadeoutTime + maximumDarknessTime, attackTimer, true);
            ...
            else if (InfernumMode.BlackFade < 1f)
            {
                ...
                float gleamAnimationCompletion = Utils.InverseLerp(blackFadeoutTime + blackFadeinTime + maximumDarknessTime, blackFadeoutTime + blackFadeinTime + maximumDarknessTime + eyeGleamTime, attackTimer, true);
        ...
        public static void DoBehavior_WandFireballs(NPC npc, Player target, ref float attackTimer, ref float armRotation, ref float frameVariant)
        {
            ...
            if (attackTimer == wandChargeUpTime && throwingWand == 0f)
            {
                Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/LightningStrike"), npc.Center);
            ...
            if (attackTimer >= wandChargeUpTime + wandAimDelay && throwingWand == 0f)
            {
                ...
                if (wandAttackCycle >= wandAimTime)
                {
                    ...
                    {
                        Main.PlaySound(SoundID.Item73, wandEnd);
                    ...
                    float aimCompletion = Utils.InverseLerp(0f, wandWaveTime, wandAttackCycle - wandAimTime, true);
                    float aimAngularOffset = (float)Math.Sin(3f * MathHelper.Pi * aimCompletion) * 1.09f;
            ...
            if (throwingWand == 1f)
            {
                ...
                if (attackTimer >= wandReelBackTime)
                {
                    ...
                    if (MathHelper.Distance(WrapAngle(idealRotation), WrapAngle(armRotation)) > 0.2f)
        ...
        public static void DoBehavior_SoulSeekerResurrection(NPC npc, Player target, ref float attackTimer, ref float armRotation, ref float frameVariant)
        {
            ...
            else if (attackTimer <= redirectTime + seekerSummonTime)
            {
                ...
                float idealRotation = Utilities.Remap(attackTimer - redirectTime, 0f, seekerSummonTime, -0.54f, 0.54f);
            ...
            else
            {
                ...
                if (attackTimer == redirectTime + seekerSummonTime + seekerShootTime)
                {
					Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/PlasmaGrenadeExplosion"), target.Center);
                    ...
                    for (int i = 0; i < 35; i++)
                    {
                        ...
                        CloudParticle fireCloud = new CloudParticle(staffEnd, (MathHelper.TwoPi * i / 35f).ToRotationVector2() * 20f, fireColor, Color.DarkGray, 50, Main.rand.NextFloat(2.6f, 3.4f));
                ...
                if (attackTimer == redirectTime + seekerSummonTime + seekerShootTime + laserTelegraphTime)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        beamDirection = (WrapAngle(npc.AngleTo(target.Center) - armRotation - MathHelper.PiOver2) > 0f).ToDirectionInt();
                        npc.netUpdate = true;
                    }
                }
        ...
        public static void DoBehavior_ShadowTeleports(NPC npc, Player target, ref float attackTimer, ref float armRotation, ref float blackFormInterpolant, ref float frameVariant)
        {
            ...
            {
                float jitterInterpolant = Utils.InverseLerp(0f, jitterTime, attackTimer, true);
                npc.Center += Main.rand.NextVector2Circular(3f, 3f) * jitterInterpolant;
                npc.Opacity = Lerp(1f, 0.5f, jitterInterpolant);

                InfernumFusableParticleManager.GetParticleSetByType<ShadowDemonParticleSet>()?.SpawnParticle(target.Center + teleportOffsetAngle.ToRotationVector2() * teleportOffset + Main.rand.NextVector2Circular(20f, 20f), Main.rand.NextFloat(96f, 105f));
            }
            ...
            if (attackTimer >= jitterTime)
            {
                ...
                if (wrappedAttackTimer == 0f)
                {
                    ...
                    if (Main.netMode != NetmodeID.Server)
                    {
                        var shadowTexture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/CalamitasShadowSingleFrame");
                        shadowTexture.CreateMetaballsFromTexture(ref InfernumFusableParticleManager.GetParticleSetByType<ShadowDemonParticleSet>().Particles, npc.Center, npc.rotation, npc.scale, 28f, 10);
                    }
            ...
            blackFormInterpolant = (float)Pow(1f - npc.Opacity, 0.2f) * 3f;
        }
        ...
        public static void DoBehavior_DarkOverheadFireball(NPC npc, Player target, ref float attackTimer, ref float armRotation, ref float frameVariant)
        {
            ...
            if (fireballHasExploded == 0f)
            {
                ...
                if (canShootFire && wrappedAttackTimer <= boltShootCycleTime - 30f && wrappedAttackTimer % boltCircleReleaseRate == boltCircleReleaseRate - 1f)
                {
                    ...
                    {
                        int fireShootCount = (int)Utilities.Remap(npc.Distance(target.Center), 950f, 3000f, 24f, 48f);
                        if (fireShootCount % 2 != 0)
                            fireShootCount++;
                        if (target.Infernum_CalShadowHex().HexIsActive("Accentuation"))
                            fireShootCount -= 8;

                        float fireShootSpeed = Utilities.Remap(npc.Distance(target.Center), 800f, 3000f, 8.5f, 70f);
        ...
        public static void DoBehavior_ConvergingBookEnergy(NPC npc, Player target, ref float attackTimer, ref float armRotation, ref float frameVariant)
        {
            ...
            if (bookHasExploded == 0f)
            {
                ...
                {
                    if (attackTimer == 1f)
                        Main.PlaySound(SoundID.DD2_EtherianPortalDryadTouch, target.Center);
                    bookAppearInterpolant = Utils.InverseLerp(0f, bookAppearTime, attackTimer, true);
                ...
                else
                {
                    ...
                    GlowyLightParticle magic = new GlowyLightParticle(bookCenter + Main.rand.NextVector2Circular(8f, 2f), -Vector2.UnitY.RotatedByRandom(0.54f) * Main.rand.NextFloat(0.4f, 3f), magicColor, 20, Main.rand.NextFloat(0.09f, 0.15f), 0.87f, false);
                ...
                blackCutoutRadius = Utilities.Remap(attackTimer, bookAppearTime, bookAttackDuration, 1500f, 750f);
                if (attackTimer <= bookAppearTime)
            ...
            if (bookHasExploded == 0f)
                bookJitterInterpolant = Utils.InverseLerp(bookAppearTime + sparkReleaseTime, bookAppearTime + sparkReleaseTime + bookExplodeDelay, attackTimer, true);
            ...
            if (bookJitterInterpolant >= 1f && bookHasExploded == 0f)
            {
                ...
                if (Main.netMode != NetmodeID.Server)
                {
                    for (int i = 0; i < 35; i++)
                    {
                        ...
                        SmallSmokeParticle explosion = new SmallSmokeParticle(bookCenter + Main.rand.NextVector2Circular(64f, 64f), Main.rand.NextVector2Circular(10f, 10f) - Vector2.UnitY * 11f, explosionFireColor, Color.DarkGray, Main.rand.NextFloat(1f, 1.15f), 255f, Main.rand.NextFloatDirection() * 0.015f);
                        GeneralParticleHandler.SpawnParticle(explosion);
                    }

                    for (int i = 0; i < 13; i++)
                    {
                        Gore page = Gore.NewGoreDirect(bookCenter + Main.rand.NextVector2Circular(30f, 30f), Main.rand.NextVector2Circular(7.5f, 7.5f) - Vector2.UnitY * 11f, 1007);
                        page.timeLeft = 60;
                        page.alpha = 50;
                    }
                }
        ...
        public static void DoBehavior_BrothersPhase(NPC npc, Player target, bool anyBrothers, ref float attackTimer, ref float armRotation, ref float frameVariant, ref float forcefieldScale)
        {
            ...
            forcefieldScale = Utils.InverseLerp(42f, 0f, attackTimer, true);
            ...
            if (attackTimer == rumbleTime)
            {
                ...
                {
                    var shadowTexture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/CalamitasShadowSingleFrame");
                    shadowTexture.CreateMetaballsFromTexture(ref InfernumFusableParticleManager.GetParticleSetByType<ShadowDemonParticleSet>().Particles, npc.Center, npc.rotation, npc.scale, 18f, 10);
                }
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    for (int i = 0; i < 50; i++)
                    {
                        ...
                        NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<SoulSeeker2>(), npc.whoAmI, seekerAngle);
                    }
                }
            }
        ...
        public static void DoBehavior_TransitionToFinalPhase(NPC npc, Player target, ref float attackTimer, ref float armRotation, ref float eyeGleamInterpolant, ref float frameVariant)
        {
            ...
            float blackFadeCompletion = Utils.InverseLerp(teleportDelay - blackFadeTime, teleportDelay + blackFadeTime, attackTimer, true);
            float blackFadeSpike = CalamityUtils.Convert01To010(blackFadeCompletion);
            InfernumMode.BlackFade = (float)Sqrt(blackFadeSpike);
        ...
        public static void DoBehavior_BarrageOfArcingDarts(NPC npc, Player target, ref float attackTimer, ref float armRotation, ref float frameVariant)
        {
            ...
            float flySlowdownInterpolant = Utils.InverseLerp(attackTransitionDelay, 0f, attackTimer - shootDelay - shootTime, true);
            Vector2 armStart = npc.Center + new Vector2(npc.spriteDirection * 9.6f, -2f);
            Vector2 armEnd = armStart + (armRotation + MathHelper.PiOver2).ToRotationVector2() * npc.scale * ArmLength;
        ...
        public static void DoBehavior_DeathAnimation(NPC npc, Player target, ref float attackTimer, ref float armRotation, ref float eyeGleamInterpolant, ref float forcefieldScale, ref float drawCharredForm, ref float frameVariant)
        {
            ...
            if (attackTimer >= textDelay && attackTimer < textDelay + chargeUpTime)
            {
                ...
                SquishyLightParticle energy = new SquishyLightParticle(energySpawnPosition, (npc.Center - energySpawnPosition) * 0.045f, 1.18f, energyColor, 48, 1f, 1.9f, 3f, 0.003f);
            ...
            if (drawCharredForm == 1f)
            {
				...
				npc.modNPC.music = InfernumMode.Instance.GetSoundSlot(SoundType.Music, "Sounds/Music/Nothing");
                Main.musicFade[(InfernumMode.CalamityMod as CalamityMod.CalamityMod).GetMusicFromMusicMod("CalamitasClone") ?? 0] = -0.1f;
            }
            else
                npc.spriteDirection = (target.Center.X < npc.Center.X).ToDirectionInt();
            ...
            if (attackTimer == textDelay + chargeUpTime + 196f)
            {
                ...
                {
                    var shadowTexture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/CalamitasShadowSingleFrame");
                    shadowTexture.CreateMetaballsFromTexture(ref InfernumFusableParticleManager.GetParticleSetByType<ShadowDemonParticleSet>().Particles, npc.Center, npc.rotation, npc.scale, 40f, 10);
                }
                for (int i = 0; i < 35; i++)
                {
                    Color fireColor = Main.rand.NextBool() ? Color.Yellow : Color.Red;
                    CloudParticle fireCloud = new CloudParticle(npc.Center, (MathHelper.TwoPi * i / 35f).ToRotationVector2() * 20f, fireColor, Color.DarkGray, 50, Main.rand.NextFloat(2.6f, 3.4f));
                    GeneralParticleHandler.SpawnParticle(fireCloud);
                }
        ...
        public static void CreateCastAnimationParticles(NPC npc)
        {
            ...
            FlareShine particle = new FlareShine(npc.Center + Main.rand.NextVector2Circular(40f, 60f) - Vector2.UnitY * 12f, particleVelocity, particleColor, bloomColor, Main.rand.NextFloat(-0.1f, 0.1f), particleScale, particleScale * 1.8f, 30, 0f, 12f, 0f, 6);
        ...
        public static void CreateCartoonAngerParticles(NPC npc)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                CartoonAngerParticle angy = new CartoonAngerParticle(angerParticleSpawnPosition, Color.Red, Color.DarkRed, angerParticleLifetime, Main.rand.NextFloat(MathHelper.TwoPi), angerParticleScale);
        ...
        public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            CalShadowFrameType frameVariant = (CalShadowFrameType)npc.Infernum().ExtraAI[FrameVariantIndex];
            Texture2D texture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/CalamitasShadow");
            Texture2D armTexture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/CalamitasShadowArm");
            Vector2 origin = npc.frame.Size() * 0.5f;

            // Incorporate the black shadow form effects.
            lightColor = Color.Lerp(lightColor, Color.Black, (float)Pow(npc.localAI[2], 0.33f));
            bool drawCharredForm = npc.Infernum().ExtraAI[DrawCharredFormIndex] == 1f;
            float shadowBackglowOffset = (float)Pow(npc.localAI[2], 2.8f) * npc.scale * 25f;
            ...
            if (shadowBackglowOffset > 0f && !drawCharredForm && npc.Opacity > 0f)
            {
                for (int i = 0; i < 10; i++)
                {
                    ...
                    Main.spriteBatch.Draw(texture, drawPosition + drawOffset, npc.frame, lightColor * (1f - npc.localAI[2]), npc.rotation, origin, npc.scale, direction, 0f);
                }
            }
            ...
            if (!drawCharredForm)
            {
                for (int i = 0; i < 5; i++)
                {
                    ...
                    Color backglowColor = Color.Purple;
            ...
            if (npc.ai[0] == (int)CalShadowAttackType.WandFireballs && npc.Infernum().ExtraAI[2] == 0f)
            {
                ...
                float wandRotation = armRotation + MathHelper.Pi - PiOver4;
                Texture2D wandTexture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/CharredWand");
                ...
                if (wandBrightness > 0f)
                {
                    for (int i = 0; i < 6; i++)
                    {
                        Color wandMagicBackglowColor = Color.HotPink * wandBrightness * npc.Opacity * 0.6f;
            ...
            {
                float staffRotation = armRotation + MathHelper.Pi - PiOver4;
                float telegraphInterpolant = npc.Infernum().ExtraAI[1];
                Texture2D staffTexture = ModContent.GetTexture("CalamityMod/Items/Weapons/Summon/BlightedEyeStaff");
                ...
                BloomLineDrawInfo lineInfo = new BloomLineDrawInfo()
                {
                    ...
                    WidthFactor = 0.004f + (float)Pow(telegraphInterpolant, 4f) * ((float)Math.Sin(Main.GlobalTime * 3f) * 0.001f + 0.001f),
                    ...
                    Opacity = (float)Sqrt(telegraphInterpolant),
                    BloomOpacity = 0.5f,
                    LightStrength = 5f
                };
            ...
            if (npc.ai[0] == (int)CalShadowAttackType.ConvergingBookEnergy)
            {
                ...
                if (blackCutoutRadius >= 10f && blackCutoutRadius <= 3200f)
                {
                ...
                {
                    Color bookColor = Color.Lerp(Color.HotPink, Color.White, (float)Sqrt(npc.Infernum().ExtraAI[0])) * npc.Infernum().ExtraAI[0];
                    ...
                    Texture2D bookTexture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/LashesOfChaosHeld");
                    Rectangle bookFrame = bookTexture.Frame(1, 8, 0, (int)(Main.GlobalTime * 15f) % 8);
            ...
            if (npc.ai[0] == (int)CalShadowAttackType.FireburstDashes)
            {
                ...
                Texture2D shieldTexture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/CalamitasShield");
        ...
        public static void DrawForcefield(NPC npc)
        {
            ...
            if (lifeRatio < 0.05f)
                flickerPower += 0.1f;
            float opacity = Lerp(1f, Math.Max(1f - flickerPower, 0.56f), (float)Pow(Cos(Main.GlobalTime * Lerp(3f, 5f, flickerPower)), 24f));

            // Dampen the opacity and intensity slightly, to allow the shadow to be more easily visible inside of the forcefield.
            intensity *= 0.85f;
            opacity *= 0.85f;

            Texture2D forcefieldTexture = ModContent.GetTexture("InfernumMode/ExtraTextures/ForcefieldTexture");
            forcefieldShader.UseImage("Images/Misc/Perlin");
        ...
        public static void DrawHexOnTarget(Player target, Color hexColor, float verticalOffset, float fadeInInterpolant)
        {
            ...
            if (HexStripDrawer is null) 
				HexStripDrawer = new Primitive3DStrip(HexHeightFunction, HexColorFunction);
            HexStripDrawer.UseBandTexture(ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/CalamitasShadow/ZealHexSymbols"));
