    ...
    public static class TwinsAttackSynchronizer
    {
        ...
        public static bool DoAI(NPC npc)
        {
            ...
            if (lifeRatio < Phase2LifeRatioThreshold && phase2Timer < Phase2TransitionTime)
            {
                ...
                phase2TransitionSpin *= Utils.InverseLerp(Phase2TransitionTime, Phase2TransitionTime / 2f, phase2Timer, true) * 0.5f;
                ...
                if (Main.netMode != NetmodeID.Server && phase2Timer == (int)(Phase2TransitionTime / 2))
                {
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        Gore.NewGore(npc.position, Main.rand.NextVector2Circular(6f, 6f), isRetinazer ? 143 : 144, 1f);
                    ...
                    Main.PlaySound(InfernumSoundRegistry.ApolloMissileLaunch, Target.Center);
                }
        ...
        public static void DoBehavior_ChargeRedirect(NPC npc, bool isSpazmatism)
        {
            ...
            npc.Center = Vector2.Lerp(npc.Center, hoverDestination, Utils.InverseLerp(npc.Distance(hoverDestination), 96f, 360f) * 0.0425f);
        ...
        public static void DoBehavior_DownwardCharge(NPC npc)
        {
            ...
            if (UniversalAttackTimer < chargeDelay - 1f)
            {
                if (UniversalAttackTimer == 1f)
                    Main.PlaySound(InfernumSoundRegistry.ArtemisApolloDashTelegraph, npc.Center);
        ...
        public static void DoBehavior_SwitchCharges(NPC npc, bool isSpazmatism, bool isRetinazer, ref float chargingFlag)
        {
            ...
            if (chargeSpecificWrappedAttackTimer <= redirectTime)
            {
                ...
                {
                    Vector2 idealVelocity = npc.SafeDirectionTo(hoverDestination) * (float)Math.Min(32f, npc.Distance(hoverDestination));
        ...
        public static void DoBehavior_FlamethrowerBurst(NPC npc, bool isSpazmatism, bool isRetinazer, ref float afterimageInterpolant)
        {
            ...
            if (UniversalAttackTimer < redirectTime + attackTelegraphTime)
            {
                ...
                if (isSpazmatism)
                {
                    ...
                    if (UniversalAttackTimer == redirectTime + 1f)
                        Main.PlaySound(InfernumSoundRegistry.ArtemisApolloDashTelegraph, Target.Center);

                    // Reel back.
                    float reelBackSpeed = MathHelper.Lerp(3f, 20f, Utils.InverseLerp(16f, attackTelegraphTime - 10f, UniversalAttackTimer - redirectTime, true));
            ...
            if (isSpazmatism)
            {
                ...
                if (chargeTimer <= 1f)
                {
                    Main.PlaySound(InfernumSoundRegistry.ArtemisApolloDash, Target.Center);
                    npc.velocity = npc.SafeDirectionTo(Target.Center) * chargeSpeed;
                    npc.netUpdate = true;
                }
            ...
            if (isRetinazer)
            {
                ...
                telegraphOpacity = Utils.InverseLerp(0f, telegraphTime * 0.75f, telegraphShootTimer, true);
                ...
                else if (!heldItem.Calamity().trueMelee)
        ...
        public static bool DoBehavior_ChaoticFireAndDownwardLaser(NPC npc, bool isSpazmatism)
        {
            ...
            Vector2 nextCenterOfMass = new Vector2(centerOfMassX, centerOfMassY);
            ...
            if (isSpazmatism)
            {
                float slowdownFactor = Utils.InverseLerp(attackCycleTime - slowdownTime, attackCycleTime - 1f, attackCountdown, true);
                if (isAttacking)
                    attackCountdown--;
                else
                    slowdownFactor = 1f;
                ...
                // Update the spin.
                spinRotation += Utils.InverseLerp(0f, shootDelay * 0.5f, UniversalAttackTimer, true) * slowdownFactor * maxSpinAngularVelocity * (Target.Center.X > npc.Center.X).ToDirectionInt();
            ...
            if (isSpazmatism && readyToAttack && Vector2.Distance(nextCenterOfMass, hoverDestination) <= 75f && !isAttacking && pointingUpwards)
            {
            ...
            if (!isSpazmatism && isAttacking)
            {
                ...
                {
                    float chargeUpInterpolant = Utils.InverseLerp(attackCycleTime, attackCycleTime - laserChargeUpTime, attackCountdown, true);
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        ...
                        SquishyLightParticle laserEnergy = new SquishyLightParticle(laserEnergySpawnPosition, laserEnergyVelocity, 1.5f, laserEnergyColor, 36, 1f, 4f);
        ...
        public static void DoBehavior_DeathAnimation(NPC npc)
        {
            ...
            if (npc.Infernum().ExtraAI[3] == 1f && otherTwinExists)
            {
                npc.damage = 0;
                npc.dontTakeDamage = true;
                npc.Opacity = Utils.InverseLerp(56f, 0f, UniversalAttackTimer, true);
            ...
            if (UniversalAttackTimer == 1f)
                Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/AresEnraged"));
            ...
            switch ((RetinazerAttackState)(int)attackState)
            {
                case RetinazerAttackState.SwiftLaserBursts:
                    ...
                    if (attackTimer <= laserShootDelay)
                    {
                        ...
                        for (int i = 0; i < 2; i++)
                        {
                            ...
                            Vector2 laserEnergySpawnPosition = endOfLaserCannon + Main.rand.NextVector2Unit() * Main.rand.NextFloat(80f, 132f);
                            Vector2 laserEnergyVelocity = (endOfLaserCannon - laserEnergySpawnPosition) * 0.04f;
                            SquishyLightParticle laserEnergy = new SquishyLightParticle(laserEnergySpawnPosition, laserEnergyVelocity, 0.8f, laserEnergyColor, 30, 1f, 4f);
                            GeneralParticleHandler.SpawnParticle(laserEnergy);
                        }
                    }
                    ...
                    else
                    {
                        ...
                        {
                            Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/PlasmaBolt"), Target.Center);
                        ...
                        {
                            Main.PlaySound(InfernumSoundRegistry.ExoLaserShootSound, Target.Center);
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                ProjectileSpawnManagementSystem.PrepareProjectileForSpawning(laser =>
                                {
                ...
                case RetinazerAttackState.AgileLaserbeamSweeps:
                    ...
                    if (attackTimer <= chargeUpTime)
                    {
                        ...
                        float turnSpeedFactor = Utilities.Remap(attackTimer, 0f, 7f, 16f, 1f);
                        ...
                        telegraphOpacity = Utils.InverseLerp(0f, chargeUpTime * 0.6f, attackTimer, true);
                        telegraphDirection = npc.rotation + MathHelper.PiOver2;
                    }
        ...
        public static void DoBehavior_SpazmatismAlone(NPC npc, ref float chargingFlag)
        {
            ...
            switch ((SpazmatismAttackState)(int)attackState)
            {
                ...
                case SpazmatismAttackState.CursedFlameSpin:
                    ...
                    float windUpInterpolant = Utils.InverseLerp(0f, windUpTime, attackTimer, true);
            ...
            if (InPhase2)
            {
                switch (UniversalStateIndex)
                {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        CurrentAttackState = TwinsAttackState.FlamethrowerBurst;
                        break;
                    case 5:
