    ...
    public class ProvidenceAttackerGuardianBehaviorOverride : NPCBehaviorOverride
    {
        ...
        public override bool PreAI(NPC npc)
        {
            ...
            if (providence.Infernum().ExtraAI[3] == 1f && Collision.SolidCollision(npc.TopLeft, npc.width, npc.height))
            {
                ...
                Main.PlaySound(InfernumSoundRegistry.ProvidenceLavaEruptionSound, npc.Center);

                Color ashColor = IsEnraged ? Color.Teal : new Color(255, 191, 73);
                for (int j = 0; j < (InfernumConfig.Instance.ReducedGraphicsConfig ? 20 : 100); j++)
                {
        ...
        public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            if (IsEnraged)
            {
                texture = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/Providence/ProvidenceAttackerGuardianNight");
                glowmask = ModContent.GetTexture("InfernumMode/BehaviorOverrides/BossAIs/Providence/ProvidenceAttackerGuardianNightGlow");
            }
