		...
		public override bool PreAI(NPC npc)
        {
            ...
            if (attackTimer > shootRate)
            {
                for (int i = 0; i < Main.maxNPCs; i++)
                {
                    ...
                    {
                        if (seeker == npc)
							Main.PlaySound(InfernumMode.CalamityMod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/SCalSounds/BrimstoneShoot"), SCal.Center);
                        break;
                    }
                }
            ...
            bool dontShoot = scalAttack == SCalAttackType.CondemnationFanBurst || scalAttack == SCalAttackType.ExplosiveCharges || scalAttack == SCalAttackType.FireLaserSpin || scalAttack == SCalAttackType.BrimstoneJewelBeam;
