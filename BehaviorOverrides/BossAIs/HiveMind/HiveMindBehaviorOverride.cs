    ...
    public class HiveMindBehaviorOverride : NPCBehaviorOverride
    {
        ...
        public static void DoBehavior_SuspensionStateDrift(NPC npc, Player target, float lifeRatio, ref float attackTimer)
        {
            ...
            if (attackTimer >= driftTime)
            {
                ...
                bool shouldBecomeInvisible =
					nextAttack == HiveMindAttackState.NPCSpawnArc ||
					nextAttack == HiveMindAttackState.SpinLunge ||
					nextAttack == HiveMindAttackState.CloudDash ||
					nextAttack == HiveMindAttackState.UndergroundFlameDash ||
					nextAttack == HiveMindAttackState.EaterOfSoulsWall ||
					nextAttack == HiveMindAttackState.CursedRain ||
					nextAttack == HiveMindAttackState.BlobBurst;
                if (shouldBecomeInvisible)
                {
                    if (nextAttack == HiveMindAttackState.EaterOfSoulsWall || nextAttack == HiveMindAttackState.CursedRain)
                    {
        ...
        public static void DoBehavior_SpinLunge(NPC npc, Player target, bool enraged, float lifeRatio, ref float fadeoutCountdown, ref float slowdownCountdown, ref float attackTimer)
        {
            ...
            if (attackTimer <= teleportDelay)
            {
                ...
                npc.Opacity = Utils.InverseLerp(teleportDelay - 16f, 0f, attackTimer, true);
                npc.velocity *= 0.94f;
                npc.dontTakeDamage = true;
                return;
            }

            spinIncrement += (float)Math.Pow(Utils.InverseLerp(MaxSlowdownTime + LungeSpinChargeDelay * 0.85f, MaxSlowdownTime, attackTimer - teleportDelay, true), 0.6f);
        ...
        public static void DoBehavior_CloudDash(NPC npc, Player target, bool enraged, float lifeRatio, ref float slowdownCountdown, ref float attackTimer)
        {
            ...
            if (attackTimer <= teleportDelay)
            {
                ...
                if (attackTimer == 1f)
                {
                    ...
                    teleportPosition.Y -= RainDashOffset * MathHelper.Lerp(1.2f, 1.5f, Utils.InverseLerp(1f, 0.4f, lifeRatio, true));
                ...
                CreateTeleportTelegraph(new Vector2(teleportPositionX, teleportPositionY));

                npc.Opacity = Utils.InverseLerp(teleportDelay - 8f, 0f, attackTimer, true);
                npc.velocity *= 0.94f;
            ...
            if (npc.alpha > 0)
            {
                ...
                if (npc.alpha <= 0)
                {
                    ...
                    npc.velocity *= MathHelper.Lerp(1f, 1.3f, Utils.InverseLerp(1f, 0.4f, lifeRatio, true));
                    if (enraged)
                        npc.velocity *= 1.5f;
                    if (BossRushEvent.BossRushActive)
                        npc.velocity *= 2.4f;
        ...
        public static void DoBehavior_UndergroundFlameDash(NPC npc, Player target, bool enraged, float lifeRatio, ref float attackTimer)
        {
            ...
            if (attackTimer <= teleportDelay)
            {
                npc.Opacity = Utils.InverseLerp(teleportDelay - 6f, 0f, attackTimer, true);
        ...
        public static void CreateTeleportTelegraph(Vector2 teleportPosition, float cloudOpacity = 1f)
        {
            ...
            CloudParticle noxiousCloud = new CloudParticle(teleportPosition, Main.rand.NextVector2Circular(6f, 6f), fireColor * cloudOpacity, Color.DarkGray, 120, Main.rand.NextFloat(2f, 2.4f));
