...
{
    public class PrimitiveBatchingSystem : IHookEdit
    {
        ...
        internal static Dictionary<Type, PrimitiveBatch> Batches = new Dictionary<Type, PrimitiveBatch>();

        public void Load()
        {
            Batches = new Dictionary<Type, PrimitiveBatch>();
        ...
        public static bool BatchIsRegistered<T>() =>
            Batches.ContainsKey(typeof(T));

        public static void PrepareBatch<T>(PrimitiveTrailCopy trail)
        {
            Type type = typeof(T);
            ...
            Batches[type] = new PrimitiveBatch
            {
                PrimitiveDrawer = trail,
                Indices = new List<short>(),
                Vertices = new List<PrimitiveTrailCopy.VertexPosition2DColor>()
            };
        }

        public static void PrepareVertices<T>(IEnumerable<Vector2> originalPositions, Vector2 generalOffset, int totalTrailPoints, float? directionOverride = null)
        {
        ...
        public static void PrepareVertices<T>(List<short> triangleIndices, List<PrimitiveTrailCopy.VertexPosition2DColor> vertices)
        {
            // Don't attempt to prepare anything if the batch has not been created yet.
            Type type = typeof(T);
            if (!Batches.TryGetValue(type, out PrimitiveBatch batch))
