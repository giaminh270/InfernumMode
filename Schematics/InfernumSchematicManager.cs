...
namespace InfernumMode.Schematics
{
    public static class InfernumSchematicManager
    {
		...
		internal static void Load()
        {
			...
			{
				["Profaned Arena"] = InfernumSchematicIO.LoadSchematic("Schematics/ProfanedArena.csch"),
			};				
        }
		...
		public static void PlaceSchematic<T>(string name, Point pos, SchematicAnchor anchorType, ref bool specialCondition, T chestDelegate = null) where T : Delegate
		{
			...
			{
				InfernumMode.Instance.Logger.Warn($"Tried to place a schematic with name \"{name}\". No matching schematic file found.");
				return;
			}
			...
			{
				InfernumMode.Instance.Logger.Warn("Schematic failed to place: Part of the target location is outside the game world.");
				return;
			}
