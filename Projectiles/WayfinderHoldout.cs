...
namespace InfernumMode.Projectiles
{
    public class WayfinderHoldout : ModProjectile
    {
        ...
        public static bool IsGateSet => PoDWorld.WayfinderGateLocation != Vector2.Zero;
        ...
        public void DoBehavior_Teleport()
        {
            ...
            Vector2 baseOffset = new Vector2(20 * projectile.spriteDirection, -15);

            // Why is this a problem?
            Owner.fallStart = (int)(Owner.position.Y / 16f);

            if (IsGateSet && Main.myPlayer == projectile.owner)
                MoonlordDeathDrama.RequestLight(Utils.InverseLerp(30f, 64f, Time, true), Owner.Center);

            if (Time == 0)
            {
                if (IsGateSet)
                    Main.PlaySound(InfernumSoundRegistry.WayfinderTeleport, Owner.Center);
                else
                    Main.PlaySound(InfernumSoundRegistry.WayfinderFail, Owner.Center);
                SoundSlot = SlotId.Invalid;
            }

            // Periodically emit dust, increasing over time to indicate charging up
            if (Main.netMode != NetmodeID.MultiplayerClient && Time % dustSpawnRate == 0 && Time < TeleportationTime)
            {
                CreateFireDust(projectile.Center, Main.rand.NextFloat(2f, 2.5f));
            }

            if (Time >= SpinDelay && Time < TeleportationTime)
            {
                if (Time <= SpinMaxTime)
                {
            ...
            if (Time > 70 && Time < TeleportationTime && IsGateSet)
            ...
            if (Time == TeleportationTime)
            {
                ...
                {
                    Owner.Bottom = PoDWorld.WayfinderGateLocation;
        ...
        public void DoBehavior_Create()
        {
            ...
            if (Time == 0)
                Main.PlaySound(InfernumSoundRegistry.WayfinderCreateSound, Owner.Center);
            SoundSlot = SlotId.Invalid;

            // Periodically emit dust, increasing over time to indicate charging up
            if (Main.netMode != NetmodeID.MultiplayerClient && Time % dustSpawnRate == 0 && Time < CreationTime)
                CreateFireDust(projectile.Center, Main.rand.NextFloat(2f, 2.5f));
            ...
            if (Time == CreationTime)
            {
                ...
                if (tipTile.IsTileSolid())
                {
                    ...
                    PoDWorld.WayfinderGateLocation = playerBottom.ToWorldCoordinates(8, 0);
                }
                else
                {
                    ...
                    if (newBottom != GenSearch.NOT_FOUND)
                        PoDWorld.WayfinderGateLocation = newBottom.ToWorldCoordinates(8, 0);
                    else
                        return;
                }
                ...
                for (int i = 0; i < 60; i++)
                {
                    CreateFireDust(tip, Main.rand.NextFloat(4f, 8f));
                    Color color = Main.rand.NextBool() ? WayfinderSymbol.Colors[1] : WayfinderSymbol.Colors[2];
                    Particle particle = new GenericSparkle(PoDWorld.WayfinderGateLocation, (Vector2.One * Main.rand.NextFloat(1f, 5f)).RotatedByRandom(MathHelper.TwoPi), color, WayfinderSymbol.Colors[0], Main.rand.NextFloat(0.2f, 0.4f), 60, Main.rand.NextFloat(-0.1f, 0.1f));
                    GeneralParticleHandler.SpawnParticle(particle);
                }

                // Create a bunch of fire at the creation point.
                CreateFlameExplosion(PoDWorld.WayfinderGateLocation, 40, 40, 40, 1);
        ...
        public void DoBehavior_Destroy()
        {
            ...
            {
                if (IsGateSet)
                    Main.PlaySound(InfernumSoundRegistry.WayfinderDestroySound, Owner.Center);
                else
                    Main.PlaySound(InfernumSoundRegistry.WayfinderFail, Owner.Center);
                SoundSlot = SlotId.Invalid;
            }
            ...
            Vector2 tip = GetTip();
            if (Main.netMode != NetmodeID.MultiplayerClient && Time % dustSpawnRate == 0 && Time < DestructionTime)
            {
                CreateFireDust(projectile.Center, Main.rand.NextFloat(2f, 2.5f));
                if (IsGateSet)
            ...
            if (Time == DestructionTime)
            {
                if (IsGateSet)
                {
                    ...
                    PoDWorld.WayfinderGateLocation = Vector2.Zero;
        ...
        public static void CreateFireDust(Vector2 position, float speed, Vector2? positionOffset = null)
        {
            ...
            int dustType = PoDWorld.WayfinderGateLocation == Vector2.Zero ? DustID.RedTorch : DustID.IceTorch;
        ...
        public void DrawShaderOverlay(Texture2D texture, Texture2D glowTexture, Rectangle sourceRectangle, Color lightColor, float opacity, Vector2 origin, SpriteEffects spriteEffects)
        {
            ...
            DrawData drawData = new DrawData(texture, projectile.Center - Main.screenPosition, sourceRectangle, lightColor * opacity * projectile.Opacity, projectile.rotation, origin, projectile.scale * 0.75f, spriteEffects, 0);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            else if (CurrentUseContext == UseContext.Create && Time > UpwardsMovementTime)
            {
            ...
            else if (CurrentUseContext == UseContext.Destroy && Time < DestroyMaxTime)
            {
            ...
            // Main drawing
            Main.spriteBatch.Draw(texture, projectile.Center - Main.screenPosition, sourceRectangle, lightColor * projectile.Opacity, projectile.rotation, origin, projectile.scale * 0.75f, spriteEffects, 0);
            Main.spriteBatch.Draw(glowTexture, projectile.Center - Main.screenPosition, sourceRectangle, Color.White * projectile.Opacity, projectile.rotation, origin, projectile.scale * 0.75f, spriteEffects, 0);
            ...
            else if (CurrentUseContext == UseContext.Destroy && Time < DestroyMaxTime && IsGateSet)
            {
                float interpolant = Time / DestroyMaxTime;
