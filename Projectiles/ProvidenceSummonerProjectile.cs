...
namespace InfernumMode.Projectiles
{
    public class ProvidenceSummonerProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Time == 115f)
            {
                Main.PlaySound(InfernumSoundRegistry.LeviathanRumbleSound, projectile.Center);
                Main.PlaySound(InfernumSoundRegistry.ProvidenceSpawnSuspenseSound);
            }
        ...
        public override void Kill(int timeLeft)
        {
            ...
            Main.PlaySound(InfernumSoundRegistry.ProvidenceScreamSound);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                Color color = Color.Lerp(new Color(1f, 0.62f, 0f, 0f), Color.White, (float)Math.Pow(projectile.Opacity, 2.7f)) * (float)Math.Pow(projectile.Opacity, 2f);
            ...
            float glimmerInterpolant = Utils.InverseLerp(40f, 20f, projectile.timeLeft, true) * Utils.InverseLerp(0f, 12f, projectile.timeLeft, true) * 0.56f;

            if (glimmerInterpolant > 0f)
            {
                Main.spriteBatch.SetBlendState(BlendState.Additive);

                texture = ModContent.GetTexture("InfernumMode/ExtraTextures/GreyscaleObjects/LargeStar");
