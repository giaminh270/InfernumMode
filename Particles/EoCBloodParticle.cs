...
namespace InfernumMode.Particles
{
    public class EoCBloodParticle : Particle
    {
        ...
        public override void Update()
        {
            ...
            Color = Color.Lerp(InitialColor, Color.Transparent, (float)Math.Pow(LifetimeCompletion, 3f));
            Rotation = Velocity.ToRotation() + MathHelper.PiOver2;
        }

        public override void CustomDraw(SpriteBatch spriteBatch)
        {
            float verticalStretch = Utils.InverseLerp(0f, 24f, Math.Abs(Velocity.Y), clamped: true) * 0.84f;
            float brightness = (float)Math.Pow(Lighting.Brightness((int)(Position.X / 16f), (int)(Position.Y / 16f)), 0.15f);
